...
- `MainWindowPanelService` assembles the `MoleculeInspectorDock` from **View ▸ Molecule Info** or the toolbar cube toggle. The dock embeds the existing `Preview3D`, preserves selection-scoped conversion and XYZ export, pauses on hide, and retains asynchronous shutdown. Atom counts include generated hydrogens; ring counts use graph cycle rank.
- `CanvasView.drawForeground` paints transient angle and conservative valence feedback, never scene items. The pure valence policy lives in `features.rendering`; it neither imports RDKit nor mutates the document. Grid style/opacity and the warning toggle belong to per-canvas tool settings and are not serialized. Hex grid painting and endpoint snapping share the same geometry.

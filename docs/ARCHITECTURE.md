...
- **Services and controllers**: constructed once per canvas in `chemvas.ui.canvas_service_composer.py` with explicit keyword injection — no service locator inside services, no `=None` collaborator defaults that hide a missing wire. The composer stores cohesive legacy groups as bundles in `CanvasRuntimeServices`; a single runtime is stored directly instead of receiving a one-member bundle. The obsolete graph/tool wrapper bundles have been removed.

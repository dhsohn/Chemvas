...
- Calculation plans and artifacts (`app/chemvas/domain/document/calculation_plan.py`, `app/chemvas/features/calculation_bundle`, `app/chemvas/bootstrap/calculation_bundle.py`): the document domain owns strict Plan v1 and v2 schemas; v2 adds endpoint precomplex state. The Qt-free feature APIs own connected-component selection, semantic charge validation, endpoint-specific roles, correspondence readiness, bond changes, deterministic path prechecks, and bounded rigid-placement candidate generation. The Calculation dialog projects included atoms into one ID-based mapping table, preserves partial drafts and explicit unmapped choices, and delegates the final candidate to that same feature/domain validation path. `calculation_mapping_highlight.py` owns dialog-scoped, non-selectable atom-ID labels colored by mapping state; they never enter document serialization, selection state, or history and are cleared on every dialog exit. Bootstrap owns `.chemvas` I/O, optional legacy RDKit composition, deterministic single-file step serialization, candidate generation/inspection/selection, reactant-identity-ordered path endpoint generation, and atomic non-overwriting publication. `application.main` dispatches `inspect`, `attach-plan`, `inspect-plan`, `generate-precomplex`, `inspect-precomplex`, `select-precomplex`, and `pack-step` before importing Qt, while an argument-free invocation keeps the desktop startup path.
...
Calculation flow: headless `inspect` -> validated `.chemvas` state -> stable connected-component inventory; `attach-plan` or the Calculation dialog -> v6 document with reusable states, endpoint-specific roles, and an explicit included-atom mapping table; dialog lifetime -> transient atom-ID labels colored by mapping state; `inspect-plan` -> mapping/readiness plus path precheck. A single-component pair can proceed directly to `pack-step`. For two-component endpoints, `generate-precomplex` requires explicit contacts and environment, writes bounded deterministic rigid-placement ensembles into Calculation Plan v2, `inspect-precomplex` exposes their exact XYZ/provenance/metrics, and `select-precomplex` records one reviewed pair. `pack-step` regenerates the candidates, rejects stale or non-reproducible selections, applies charge/multiplicity and complete-bijection gates, and atomically publishes one `factory/machine-observation` v1 / `chemistry/elementary-step` v1 `machine.json`. A qualifying artifact carries reactant-identity-ordered endpoint XYZ and canonical 0-based reaction-center indices. The GUI suggests exact shared IDs and optional same-element structural mappings without mechanistic inference. Chemvas does not invent contacts, select a candidate automatically, claim optimization or stability, or replace downstream quantum optimization and scientific review.
...
- Calculation Plan v1 stores explicit states, `included`/`context_only` membership, endpoint-specific roles, and source atom correspondence. Plan v2 adds explicit step-side precomplex ensembles and reviewer selections. Neither version infers roles, contacts, or a reaction mechanism. The elementary-step handoff is one `machine.json` with a common envelope and an inline domain payload containing provenance, mapping, bond changes, and a conditional identity-ordered endpoint pair. Reviewed rigid placements remain initial guesses that require downstream quantum optimization and researcher review.
...
- `.chemvas` documents are versioned (current: v6; v1–v5 stay loadable). v4 made bond arrays compact, v5 added the optional Calculation Plan v1, and v6 allows Calculation Plan v2 precomplex state. Deleted-slot tombstones (`null` entries in pre-v4 files) remain runtime bookkeeping and never reach new documents. Bond identity is runtime-scoped — the calculation plan references stable atom ids and complete connected-component atom-id sets, not bond positions.

...
데스크톱 시작과 헤드리스 출력은 글꼴에 고정 96 논리 DPI를 사용하므로 화면의
72/96/144 DPI 설정이 결합 대비 글자 크기를 바꾸지 않습니다. 화면의 장치 픽셀
배율은 별개입니다. 글꼴을 번들하거나 서로 다른 Qt/글꼴 설치 간 기하가 같음을
보장하지 않으므로 최종 출력을 확인하세요.

...
예산은 그려진 배치 범위를 설명하며, 전체 출력의 바깥 여백이나 노트에
남아 있는 배치 상자 공간은 포함하지 않습니다. 원자 라벨과 붙은 화살표 라벨 출력은
실제 글리프 범위를 씁니다. 비트맵·컬러 글꼴 테이블이 감지된 화살표 라벨은 기존
배치 상자 범위를 유지합니다. 나열되지 않은 객체는 여전히 제자리에 남고 전체 출력을

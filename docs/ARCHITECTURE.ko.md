...
도구 명령과 캔버스 알림은 `MainWindowToolStateService`에서 만난다. UI 선택을
한 번의 캔버스 모드 변경으로 전달하고, 완료 콜백에서 체크된 액션·상태 안내·옵션 바를
반영한다. 별도의 context-page 전달 서비스는 제거했다. 위젯 팩터리는 분리하고,
Ring Fill은 Select의 옵션 페이지 override로 유지한다. 히스토리 갱신은 새 도구를
알리지 않고 복구된 주석 설정을 반영한다.

미리보기 키는 안정된 원자 ID를 기준으로 한 상대 2D 좌표(평행 이동의 부동소수점
오차를 흡수하도록 소수점 아홉 자리 반올림), 화학 구조 및 실제 전하·라디칼 주석으로
구성한다. 평행 이동은 완성된 결과와 실행 중 요청을 유지한다. 상대 배치, 쐐기·해시,
그림의 이중결합 입체정보 변경은 키를 무효화하므로 좌표 자체를 빼면 안 된다.
변환 실패와 pause/clear는 기존처럼 키를 지워 재시도와 최신 선택 반영을 허용한다.

캔버스마다 `ValenceWarningCache` 하나를 파생 뷰 상태로 소유한다. 전경을 그릴 때
좌표·색상을 제외한 실제 화학 입력을 비교하여 불변 경고 집합을 재사용한다. 키 읽기는
원자·결합 수에 선형이지만, 결합차수 합계 생성과 원자가 분류는 반복하지 않는다.
그래프 인덱스 버전만으로는 원소·전하 편집을 감지할 수 없다. 값 비교는 별도 변경
알림 체계 없이 모델 교체·Undo·정확한 롤백도 처리한다. 캐시는 문서 원본이나 장면
내보내기·복구 스냅샷에 포함되지 않는다.

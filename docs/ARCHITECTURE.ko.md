...
`MoleculeModel`은 원자와 결합을 Qt 없는 데이터로 소유한다. 문서가 저장하는 나머지
그려지는 객체 — 고리 채움, 노트, 마크, 화살표와 선, TS 괄호, 도형, 오비탈, 이미지 —
는 아직 문서를 쓸 때
...
아이템을 그린다(`render_shape_item`, 도형 아이템의 칠을 쓰는 유일한 곳). 도형 state를
읽는 모든 곳 — 문서 저장, undo 캡처, 클립보드, 삭제 캡처, 뒤집기와 회전, 레이아웃 검사
— 은 `shape_state_dict_for`를 통해 레코드에서 읽는다. 레코드 없는 부착 도형은 붓에서

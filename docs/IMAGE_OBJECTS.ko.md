...
그룹이나 선택에 이미지가 여러 개 있으면 픽셀 크기와 캔버스 위치가 적힌 번호
목록에서 대상을 먼저 고릅니다. 해당 이미지만 바뀌고 그룹과 선택은 유지됩니다.
...
다른 파일 형식은 삽입 전에 PNG 또는 JPEG로 변환하세요. 확장자만 바꾸는 것은
변환이 아닙니다. 인식하지 못하거나 지원하지 않는 바이트는 인식된 PNG/JPEG의
손상 오류와 구별해 안내합니다. 구성 중 개별 이미지 오류에는 0부터 시작하는 항목 번호가
표시됩니다. 예를 들어 `image 2`는 세 번째 이미지입니다.
...
실행 취소/다시 실행에 참여합니다. 새 이미지는 도형과 고리 채움 위, 원본 라벨과
화살표 아래에서 시작합니다. 이미지·도형 또는 둘을 함께 선택하고
**Edit → Bring to Front**(맨 앞으로)나 **Edit → Send to Back**(맨 뒤로)을
선택하면 그림 내 순서를 바꿀 수 있습니다. 선택 내부의 상대 순서와 변경된 순서는
저장·다시 열기, 복사·붙여넣기, 내보내기, 실행 취소·다시 실행에도 유지됩니다.
크기 조절은 속성 대화상자로 하며 이미지 모서리 핸들은 없습니다. 그룹 회전과
뒤집기는 픽셀을 똑바로 둔 채 이미지 사각형만 재배치합니다. 픽셀 회전,
반전, 자르기, 필터는 지원하지 않습니다. 텍스트는 **Edit → Rotate…** 또는
선택 회전 핸들로 회전하며, 배경 상자도 함께 회전합니다. 회전 각도는 저장과
내보내기에 반영됩니다. 자동 **Arrange Scheme**은 이미지를 포함한
...
PNG의 첫 IEND 청크 뒤 데이터(마지막 줄바꿈이나 두 번째 IEND 포함)는 후행 데이터
오류로 거부합니다. 깨끗한 PNG로 다시 내보내세요. Chemvas는 가져오기 때 원본
바이트를 잘라내거나 다시 쓰지 않습니다.
...
일반 SVG와 벡터 클립보드 출력을 포함해, 렌더링된 래스터에는 PNG 텍스트
청크나 JPEG 주석 같은 원본 이미지 텍스트 메타데이터를 넣지 않습니다.
네이티브 문서와 Editable Chemvas SVG는 이 메타데이터를 포함한 원래 인코딩
바이트를 그대로 보존합니다. 원본 이미지 메타데이터를 편집 가능한 문서와
함께 공유하지 않으려면 일반 그림 출력을 사용하세요.
...
CLI 구성의 텍스트에는 선택적으로 `rotation` 각도(도)를 지정할 수 있습니다.
유한한 숫자이며 `(x, y)` 기준점을 중심으로 캔버스의 시계 방향으로 회전합니다.
이미지와 도형의 `z`는 −12부터 10 사이의 선택 값이며 클수록 앞에 그립니다.
생략하면 이미지 −2, 도형 −10의 기본 순서를 사용합니다.

Composition v1은 기존 필드로 계속 지원합니다. 텍스트 `rotation`과 그림·도형 `z`는
요청 버전 2가 필요하며, 알 수 없는 요청 버전과 필드는 거부합니다.

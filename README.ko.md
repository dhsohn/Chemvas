...
pip install "chemvas[rdkit]"     # + SMILES import, 분자식/분자량, 3D
chemvas
```
...
- `make check`가 로컬 게이트 전체를 실행합니다 — lint, 포맷, mypy,
  ...
  구조를 옮기기 전에 [CONTRIBUTING.md](CONTRIBUTING.md)를 먼저 읽으세요 —
  아키텍처 경계는 테스트로 강제됩니다.
- 문서 색인: [REFERENCE](docs/REFERENCE.md) · [AGENT_CLI](docs/AGENT_CLI.md) ·
  [ARCHITECTURE](docs/ARCHITECTURE.ko.md) · [CHANGELOG](CHANGELOG.md) ·
  [RELEASING](RELEASING.md)
- 알려진 빈칸(SDF 상호운용, 단일 파일 바이너리, 다중 분자 3D export) →
  [로드맵](docs/REFERENCE.md#roadmap--not-yet-supported)

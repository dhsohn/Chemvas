...
- 우측 고정 패널: 상단 RDKit 기반 3D preview, 하단 GFN2-xTB input/output 분석 패널
...
- `.ldraw`는 JSON 기반 포맷이며, 분자 모델/주석/화살표/TS bracket/설정값 등을 포함합니다.
  (형식: `{"type":"litedraw","version":1,"state":{...}}`)

## 단축키
- LiteDraw는 ChemDraw 호환 단축키의 주요 하위집합을 지원합니다.
...
## 의존성
- PyQt6 필요
- RDKit은 선택 사항이며, 설치된 경우 SMILES/분자식/분자량 계산, 2D -> 3D `.xyz` export, 3D preview, xTB 입력 구조 준비에 사용됩니다.
- `xtb` executable은 선택 사항이며, 설치된 경우 우측 하단 `GFN2-xTB` 패널에서 single-point/optimization 분석을 실행할 수 있습니다.
...
- RDKit이 없는 환경에서는 이 기능을 사용할 수 없습니다. LiteDraw는 기본 실행에 RDKit을 강제하지 않습니다.
...
- 대표적인 축약/alias 라벨 `Me`, `Et`, `Ph`, `OMe`, `Boc`, `CO2Me`, `t-Bu`, `i-Pr`는 3D 변환 전에 fragment로 확장됩니다.
- 지원되지 않는 라벨, 잘못 연결된 alias, wedge/hash의 잘못된 사용(예: non-single bond) 등은 명시적인 에러 메시지로 안내합니다.
- 우측 패널은 항상 표시되며 상단의 `3D Preview`에서 현재 구조를 실시간으로 확인할 수 있습니다. 마우스 드래그로 회전, 휠로 확대/축소할 수 있습니다.
- 우측 하단 `GFN2-xTB` 패널에서는 현재 selection을 input/output 구조로 캡처하고, `xtb`가 설치된 경우 input optimization, CREST, input/output pair single-point, reaction path 분석을 실행할 수 있습니다.
- xTB 결과는 별도의 노트 워크북이 아니라 새 캔버스 시트로 열립니다. 결과 시트에는 구조와 요약 note가 함께 배치됩니다.

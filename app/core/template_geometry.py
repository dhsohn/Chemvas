...
def scale_points(points: Sequence[Point2D], center: Point2D, scale: float) -> list[Point2D]:
    cx, cy = center
    return [
        (
            cx + (x - cx) * scale,
...
def regular_ring_points_for_bond(
    ...
    if occupied_polygon is not None:
        ...
        if a_in and not b_in:
            ...
            return None
    elif center_hint is not None and _manhattan_distance(center_b, center_hint) < _manhattan_distance(center_a, center_hint):
        use_center = center_b
    ...
    direction = 1.0 if _distance(p_forward, bond_end) <= _distance(p_backward, bond_end) else -1.0
    return [
        (

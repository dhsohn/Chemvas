...
class TemplatePointResolvers:
    ...
    regular_ring_points_for_bond: Callable[
        [int, int, Point2D], Sequence[Point2D] | None
    ]
    ...
    template_points_for_bond: Callable[
        [Sequence[Point2D], int, Point2D], Sequence[Point2D] | None
    ]
...
def resolve_template_insert(
    ...
    if plan.generator == "free_regular_ring":
        ...
        if plan.bond_id is None:
            return None
        points = resolvers.regular_ring_points_for_bond(
            plan.ring_size, plan.bond_id, cursor_pos
        )
    elif plan.generator == "bond_template_shape":
        if plan.bond_id is None:
            return None
        local_points = _template_shape_points(plan, (0.0, 0.0), resolvers)
        points = resolvers.template_points_for_bond(
            local_points, plan.bond_id, cursor_pos
        )
    else:
        return None

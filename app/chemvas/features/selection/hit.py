...
def choose_preferred_structure_hit(
    ...
    *,
    ...
    if (
        atom_hit is not None
        and bond_hit is not None
        and atom_score is not None
        and bond_score is not None
    ):
...
def selection_hit_matches(request: SelectionHitRequest) -> bool:
    if request.outline_hit:
        return True
    if any(
        padded_rect_contains_point(rect, request.point, pad=request.pad)

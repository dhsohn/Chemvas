...
def set_label_outline_mode(
    items: Sequence[QGraphicsItem], enabled: bool
) -> list[QGraphicsItem]:
...
@contextmanager
def exported_scene(
    scene: QGraphicsScene, export_items: Sequence[QGraphicsItem]
) -> Iterator[None]:
    expanded_export_items = export_item_closure(export_items)
    export_set = set(expanded_export_items)
    hidden = [
        item for item in scene.items() if item.isVisible() and item not in export_set
    ]

...
def append_selected_item_ids(
    canvas, atom_ids: set[int], bond_ids: set[int], item
) -> None:
...
def selected_structure_ids_for(
    canvas, *, require_non_empty: bool = False
) -> tuple[set[int], set[int]]:
...
def selected_bond_atom_ids_for(
    canvas, bond_ids: set[int]
) -> tuple[tuple[int, int], ...]:
...
def append_unique_scene_item(
    items: list, seen: set, item, *, excluded_kinds: set[str]
) -> bool:
    if item in seen:
        return False
...
def selection_items_for_copy_for(canvas) -> list:
    selected = selected_scene_items_for(
        canvas, excluded_kinds=COPY_SELECTION_EXCLUDED_KINDS
    )
    if not selected:
        return []
    items: list = []
    ...
    def add_with_children(item) -> None:
        if not append_unique_scene_item(
            items, seen, item, excluded_kinds=COPY_SELECTION_EXCLUDED_KINDS
        ):
            return

...
def apply_structure_template_command(service, key: str) -> None:
    ...
    if key in REGULAR_RING_TEMPLATES:
        ...
        service.run_recorded_build(
            _successful_template_action(
                lambda: template_builder.add_regular_ring_template(n)
            )
        )
        return
    if key in HETERO_RING_TEMPLATES:
        ...
        service.run_recorded_build(
            _successful_template_action(
                lambda: template_builder.add_hetero_ring_template(
                    n, elements, bond_orders
                )
            )
        )
        return
    if key in FUSED_BENZENE_TEMPLATES:
        ...
        service.run_recorded_build(
            _successful_template_action(
                lambda: template_builder.add_fused_benzenes(count, mode=mode)
            )
        )
        return
    if key in CROWN_ETHER_TEMPLATES:
        ...
        service.run_recorded_build(
            _successful_template_action(
                lambda: template_builder.add_crown_ether(atoms, oxygens)
            )
        )
        return

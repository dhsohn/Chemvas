...
class BondGraphicsDrawService:
    ...
    def _line_item(
        self, x1: float, y1: float, x2: float, y2: float, *, dotted: bool = False
    ):
    ...
    def one_sided_bond_strip(
        ...
        polygon = self.bold_strip_polygon(
            x1, y1, x2, y2, nx, ny, base_width, bold_width, a_id, b_id
        )
        return self.renderer.graphics.filled_polygon(polygon)

    def bold_strip_polygon(
        self, x1, y1, x2, y2, nx, ny, base_width, bold_width, a_id, b_id
    ):
        ...
        outer_start = self._miter_corner(
            x1, y1, nx, ny, outer_off, dx, dy, a_id, start_nb, bold_width
        )
        outer_end = self._miter_corner(
            x2, y2, nx, ny, outer_off, dx, dy, b_id, end_nb, bold_width
        )
        inner_end = self._miter_corner(
            x2, y2, nx, ny, inner_off, dx, dy, b_id, end_nb, bold_width
        )
    ...
    def draw_dotted_bond(
        ...
        end_x = x1 + (x2 - x1) * t1
        end_y = y1 + (y2 - y1) * t1
        path = self.renderer.dotted_bond_path(
            start_x, start_y, end_x, end_y, a_id, b_id
        )
    ...
    def draw_dotted_double_bond(
        ...
        *,
        ...
        outer_item = (
            self.renderer.graphics.path_fill(
                self.renderer.dotted_bond_path(*outer_seg, a_id, b_id)
            )
            if dotted_outer
        ...
        inner_item = (
            self.renderer.graphics.path_fill(
                self.renderer.dotted_bond_path(*inner_seg, a_id, b_id)
            )
            if not dotted_outer

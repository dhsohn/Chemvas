...
def build_panel_toolbar(
    ...
    *,
    ...
    save_as_action.triggered.connect(
        lambda _checked=False: callbacks.save_canvas_as(window)
    )
    ...
    export_figure_action.setStatusTip(
        "Export the drawing as SVG, PDF, or high-resolution PNG/TIFF"
    )
    export_figure_action.triggered.connect(
        lambda _checked=False: callbacks.export_figure(window)
    )
    window.addAction(export_figure_action)

    export_mol_action = QAction("Export MOL...", window)
    export_mol_action.setToolTip("Export MOL")
    export_mol_action.setStatusTip(
        "Export the selected structure as an MDL Molfile (.mol)"
    )
    export_mol_action.triggered.connect(
        lambda _checked=False: callbacks.export_mol(window)
    )

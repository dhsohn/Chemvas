...
if TYPE_CHECKING:
    from chemvas.ui.selection.selection_info_state import SelectionSignature


def _selection_ids_for_info(canvas) -> tuple[set[int], set[int]]:
    from chemvas.ui.selection.selection_queries import selected_chemical_ids_for
...
def emit_selection_info_for(canvas) -> None:
    state = canvas.runtime_state.selection_info_state
    callback = state.callback
    if not callback:
        return
    rotation_selection_ids = canvas.runtime_state.rotation_state.selection_ids
    ...
    if not atom_ids and not bond_ids:
        ...
        return
    if bool(canvas.rdkit.is_unavailable()):
        _blank_selection_info(state, callback)
        return
    if not bool(canvas.rdkit.is_loaded()):
    ...
    formula, mw, _ = canvas.rdkit.compute_props(submodel)
...
def maybe_warm_rdkit_for(canvas) -> None:
    ...
    if bool(canvas.rdkit.is_unavailable()):
        ...
        state.pending_signature = None
        return
    if bool(canvas.rdkit.is_loaded()):
        state.rdkit_warmup_pending = False
        state.pending_signature = None

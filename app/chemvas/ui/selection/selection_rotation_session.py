...
def _preserve_other_projected_coords(
    ports: Any,
    state: CanvasRotationState,
    previous_coords: dict[int, Coords3D],
) -> None:
    """Re-express untouched depth in the newly selected drawing camera.

    Project the stored point, not the live atom: an already stale cache must
    retain its projection error rather than becoming valid accidentally.
    These cache-only changes join the existing gesture/history payload; the
    unrelated atoms' screen positions and scene items are not moved.
    """
    for atom_id, coords in previous_coords.items():
        if atom_id in state.coord_atom_ids or ports.atom(atom_id) is None:
            continue
        bond_length = ports.canvas.renderer.style.bond_length_px
        screen = project_point_3d(
            coords,
            bond_length_px=bond_length,
            center_3d=state.start_projection_center_3d,
            anchor_2d=state.start_projection_anchor_2d,
        )
        updated = unproject_point_3d(
            screen,
            coords[2],
            bond_length_px=bond_length,
            center_3d=state.projection_center_3d,
            anchor_2d=state.projection_anchor_2d,
        )
        set_atom_coords_3d_for_id(ports.canvas, atom_id, updated)
        state.coord_atom_ids.add(atom_id)
...
def begin_axis_rotation_session(
    ...
    *,
    ...
    if atom_a is not None and atom_b is not None:
        ...
        # Like rigid rotation, construct a local drawing frame from screen XY
        # and retained depth. Raw XY may use another molecule's camera.
    ...
    state.projection_center_3d = projection_center or axis_center
    ...
    *,
    ...
    try:
        ...
        if rotating:
            ...
            _preserve_other_projected_coords(ports, state, snapshot.coords_3d)

...
import chemvas.ui.atom_coords_access as _atom_coords_access_module
import chemvas.ui.canvas_atom_graphics_state as _atom_graphics_state_module
import chemvas.ui.canvas_bond_graphics_state as _bond_graphics_state_module
import chemvas.ui.canvas_history_service as _history_service_module
import chemvas.ui.canvas_hit_testing_service as _hit_testing_service_module
import chemvas.ui.canvas_model_access as _canvas_model_access_module
import chemvas.ui.canvas_model_state as _canvas_model_state_module
import chemvas.ui.canvas_move_controller as _move_controller_module
import chemvas.ui.canvas_service_ports as _canvas_service_ports_module
import chemvas.ui.canvas_style_controller as _style_controller_module
import chemvas.ui.graphics_items as _graphics_items_module
import chemvas.ui.handle_state as _handle_state_module
import chemvas.ui.move_access as _move_access_module
import chemvas.ui.selection_outline_service as _outline_service_module
import chemvas.ui.selection_service_access as _selection_service_access_module
import chemvas.ui.selection_style_state as _selection_style_state_module
import chemvas.ui.spatial_index_state as _spatial_index_state_module
from chemvas.core.history import (
    CompositeCommand,
    HistoryCommand,
    MoveAtomsCommand,
)
from chemvas.domain.transactions import HistoryAuthoritySnapshot, RestoreOutcome
...
def _has_callback_free_qt_snapshot_ports(item: object) -> bool:
    ...
    if (
        ...
        is not object.__getattribute__
        or inspect.getattr_static(item_type, "__setattr__") is not object.__setattr__
        or (
...
def _reject_unsafe_qt_snapshot_ports(item: object) -> None:
    if isinstance(item, QGraphicsItem) and not _has_callback_free_qt_snapshot_ports(
        ...
        raise RuntimeError("Selection drag requires callback-free Qt snapshot ports")
...
def _has_trusted_selection_item_mutation_surface(item: QGraphicsItem) -> bool:
    try:
        ...
        if kind == "shape":
            return type(item.data(1)) is dict and inspect.isbuiltin(
                cast(Any, item).setPath
            )
...
class _TrustedSelectionDragSnapshot:
    ...
    objects: tuple[ObjectStateSnapshot, ...]
    containers: ContainerGraphSnapshot
    items: tuple[SceneItemExactSnapshot, ...]
    ...
    def _production_roots(
        ...
        if not _trusted_drag_module_ports_are_current():
            return None
        if not _has_exact_type(canvas, "chemvas.ui.canvas_view", "CanvasView"):
            return None
        ...
        if (
            ...
            or not _has_exact_type(
                services,
                "chemvas.ui.canvas_runtime_services",
                "CanvasRuntimeServices",
            )
            or not _has_exact_type(
                runtime_state,
                "chemvas.ui.canvas_runtime_state",
                "CanvasRuntimeState",
            )
            or not _has_exact_type(
                model,
                "chemvas.domain.document.model",
                "MoleculeModel",
            )
        ):
            return None
        move_controller = getattr(services.interaction, "move_controller", None)
        hit_testing_service = getattr(services.selection, "hit_testing_service", None)
        selection_controller = getattr(services.selection, "selection_controller", None)
        outline_service = getattr(selection_controller, "outline_service", None)
        style_controller = getattr(services.scene_operations, "style_controller", None)
        mark_registry = getattr(runtime_state, "mark_registry", None)
        if not all(
            (
                _has_exact_bound_method(
                    move_controller,
                    module="chemvas.ui.canvas_move_controller",
                ...
                _has_exact_callback_free_production_history_state(history_service),
                dict.get(
                    context_namespace,
                    "history_service",
                    _MISSING_DRAG_LOCAL,
    ...
    def capture(
        ...
        *,
        ...
        containers = ContainerGraphSnapshot()
        object_snapshots: list[ObjectStateSnapshot] = []
        ...
        for entry in atom_mapping.entries:
            ...
            if not _has_exact_type(
                ...
                return None
            snapshot = ObjectStateSnapshot.capture(entry.value, containers)
        ...
        if not all(
            (
                _has_exact_type(
                    coords_state,
                    "chemvas.ui.atom_coords_access",
                    "CanvasAtomCoords3DState",
                ),
                _has_exact_type(
                    atom_graphics_state,
                    "chemvas.ui.canvas_atom_graphics_state",
                    "CanvasAtomGraphicsState",
                ),
                _has_exact_type(
                    bond_graphics_state,
                    "chemvas.ui.canvas_bond_graphics_state",
                    "CanvasBondGraphicsState",
                ),
                _has_exact_type(
                    scene_items_state,
                    "chemvas.ui.canvas_scene_items_state",
                    "CanvasSceneItemsState",
                ),
                _has_exact_type(
                    outline_state,
                    "chemvas.ui.selection_outline_state",
                    "SelectionOutlineState",
                ),
                _has_exact_type(
                    handle_state,
                    "chemvas.ui.handle_state",
                    "CanvasHandleState",
                ),
                _has_exact_type(
                    selection_style,
                    "chemvas.ui.selection_style_state",
                    "SelectionStyleState",
                ),
                _has_exact_type(
                    spatial_index,
                    "chemvas.ui.spatial_index_state",
        ...
        affected_ring_bindings: list[tuple[QGraphicsItem, tuple[int, ...]]] = []
        ...
        if atom_ids:
            ...
            for ring in ring_items:
                ...
                if not atom_ids.isdisjoint(ring_atom_ids):
                    ...
                    affected_ring_bindings.append((ring, tuple(ring_atom_ids)))
        ...
        item_snapshots: list[SceneItemExactSnapshot] = []
        for item in unique_items:
            item_snapshot = SceneItemExactSnapshot.capture(item, containers)
        ...
        roots = (
            ...
            _DragBoundAttribute(
                selection_style,
                "suspend_outline",
                selection_style.suspend_outline,
                identity=False,
            ),
            _DragBoundAttribute(handle_state, "target", handle_state.target),
            _DragBoundAttribute(
                spatial_index, "dirty", spatial_index.dirty, identity=False
            ),
            _DragBoundAttribute(
                spatial_index, "cell_size", spatial_index.cell_size, identity=False
            ),
        ...
        return cls(
            ...
            requires_full_move=(
                ...
                or any(cast(Any, item).data(0) == "mark" for item in selection_items)
            ),
        )
    ...
    def targets_match(self, tool: SelectionDragMixin) -> bool:
        return (
            ...
            and tool._drag_boundary_bond_ids == set(self.boundary_bond_ids)
    ...
    def ports_are_trusted(self, tool: SelectionDragMixin) -> bool:
        ...
        if (
            ...
            or shift_selection_outlines_for is not _TRUSTED_SHIFT_SELECTION_OUTLINES_FOR
        ):
            return False
        ...
        if (
            ...
            return False
        if not all(
            _has_trusted_qt_item_ports(item, self.scene) for item in self.affected_items
        ):
            return False
        try:
            if not all(
                _has_trusted_selection_item_mutation_surface(cast(QGraphicsItem, item))
...
class _DragHistoryAuthority:
    ...
    history_stacks: HistoryAuthoritySnapshot
...
class _DragTransactionToken:
    ...
    history_stacks: HistoryAuthoritySnapshot | None = None
...
class _ReplacementDragCheckpoint:
    ...
    def restore_local(self, tool: SelectionDragMixin) -> None:
        ...
        tool._selection_outline_was_suspended = self.selection_outline_was_suspended
        tool._start_pos = (
...
class SelectionDragMixin:
    ...
    def _capture_drag_history_policy(
        snapshot: HistoryAuthoritySnapshot,
    ...
    def _history_checkpoint_for(
        ...
        policies = {port.name: port.value for port in authority.history_policy_ports}
        ...
        for attempt in range(2):
            try:
                if attempt == 0:
                    ...
                    policy_ports = tuple(reversed(authority.history_policy_ports))
    ...
    def _compare_and_clear_drag_token(
        ...
        *,
        ...
        if self._drag_transaction is not token:
            return False
        if authority is not None and self._drag_transaction_authority is not authority:
            return False
    ...
    def _begin_drag_transaction(
        ...
        *,
        ...
        try:
            ...
            token.history_stacks = HistoryAuthoritySnapshot.capture(history_service)
            ...
            if transaction_authority is not None:
                ...
                if history_authority is not None:
                    restored, restore_errors = self._restore_bound_history_authority(
                        history_authority
                    )
    ...
    def _release_drag_transaction(
        ...
        *,
        ...
        if (
            ...
            if not snapshot.targets_match(self) or not snapshot.ports_are_trusted(self):
                raise RuntimeError(
    ...
    def _capture_replacement_drag_checkpoint(
        ...
        try:
            ...
            return _ReplacementDragCheckpoint(
                ...
                drag_boundary_bond_ids=frozenset(self._drag_boundary_bond_ids),
                suspended_outline=self._suspended_outline,
                selection_outline_was_suspended=(self._selection_outline_was_suspended),
                start_pos=(
                    QPointF(self._start_pos) if self._start_pos is not None else None
                ),
                moved=self._moved,
    ...
    def _reapply_replacement_drag_checkpoint(
        ...
        try:
            for _attempt in range(2):
                if self._drag_transaction is not token:
                    errors.append(
                        RuntimeError("replacement drag owner changed before reapply")
                    )
                    break
                try:
                    current_history_service = self.context.history_service
    ...
    def _restore_drag_transaction(
        ...
        *,
        ...
        if replacement_token is not None:
            try:
                replacement_checkpoint = self._capture_replacement_drag_checkpoint(
                    replacement_token,
                )
            except BaseException as error:
        ...
        for _attempt in range(2):
            ...
            history_authoritative, history_errors = (
                self._restore_bound_history_authority(bound_history_authority)
            )
        ...
        consumed = self._compare_and_clear_drag_token(token) if authoritative else False
    ...
    def _push_drag_history(
        ...
        try:
            ...
            if canvas_checkpoint is not None:
                restored, restore_errors = self._restore_bound_history_checkpoint(
                    authority,
                    authority.begin_history_checkpoint,
                )
                if not restored:
        ...
        try:
            ...
            raise
        if self._drag_history_publication is not None:
            raise RuntimeError("Selection drag history publication marker was replaced")

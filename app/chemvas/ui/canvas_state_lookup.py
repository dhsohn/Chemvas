...
def ensure_canvas_state[T](
    ...
    divergence would silently hand out a fresh empty state). These states live
    as fields of the eagerly-built ``CanvasRuntimeState`` container on real
    canvases: if that container is present but lacks the field, the accessor
    and the container are out of sync (e.g. a renamed field), and creating a
    shadow copy on the canvas would split the state in two — fail loudly
    instead. Plain attribute storage remains the fallback that keeps
    lightweight test doubles working.
    """
    runtime_state = getattr(canvas, "runtime_state", None)
    if runtime_state is not None:
        ...
        if state is not None:
            return state
        if getattr(runtime_state, "STRICT_STATE_CONTAINER", False):
            msg = (

...
from chemvas.ui.preview3d.preview_3d_worker import Preview3DAdapter, Preview3DWorker
...
class Preview3D(QWidget):
    ...
    def _start_preview_worker(self) -> None:
        ...
        thread.finished.connect(
            lambda request_id=request_id: self._on_preview_thread_finished(request_id)
        )
        thread.start()
    ...
    def _position_export_xyz_button(self) -> None:
        ...
        badge_width = status_badge_width(
            badge_text, QFontMetricsF(preview_caption_font(self.font()))
        )
        badge_gap = 8.0
    ...
    def _build_copy_button(
        self, object_name: str, text: str, value_getter
    ) -> QToolButton:
        button = QToolButton(self)

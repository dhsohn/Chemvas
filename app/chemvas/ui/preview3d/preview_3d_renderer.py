...
from chemvas.ui.preview3d.preview_3d_molecule_renderer import (
...
def draw_card_shadow(
    painter: QPainter, rect: QRectF, radius: float, *, layers=None
) -> None:
...
def draw_empty_state(
    ...
    *,
    ...
    painter.drawLine(
        icon_center + QPointF(-18.0, 3.0), icon_center + QPointF(0.0, -9.0)
    )
    painter.drawLine(icon_center + QPointF(0.0, -9.0), icon_center + QPointF(19.0, 5.0))
    painter.drawLine(
        icon_center + QPointF(-18.0, 3.0), icon_center + QPointF(14.0, 18.0)
    )
    ...
    detail_rect = QRectF(
        rect.left() + 22.0, title_rect.bottom() + 3.0, rect.width() - 44.0, 34.0
    )
    ...
    *,
    ...
    painter.drawText(
        ...
        value_metrics.elidedText(
            value, Qt.TextElideMode.ElideRight, value_rect.width()
        ),
    )
    painter.restore()

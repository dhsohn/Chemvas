...
class RDKitExportJobRegistry:
    def __init__(self) -> None:
        self._owner_states: WeakKeyDictionary[Any, RDKitExportOwnerState] = (
            WeakKeyDictionary()
        )
    ...
    def _owner_state_for(self, owner: Any) -> RDKitExportOwnerState:
        ...
        if destroyed is not None:
            destroyed.connect(
                lambda *_args, state=state, registry=self: (
                    registry.disable_owner_callbacks(state)
                )
            )
        return state
    ...
    def is_latest(self, job: RDKitExportJob) -> bool:
        return (
            self._latest_generation_by_path.get(job.normalized_target_path)
            == job.generation
        )

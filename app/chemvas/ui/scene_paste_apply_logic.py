...
def apply_paste_payload(
    *,
    ...
    apply_perspective: Callable[
        [dict[int, Coords3D], Coords3D | None, Coords2D | None], None
    ]
    | None = None,
) -> PasteApplyResult:
    ...
    *,
    ...
    for entry in coords_entries:
        ...
        if not isinstance(atom_id, int) or atom_id not in atom_id_map or coords is None:
            continue
        translated_coords[atom_id_map[atom_id]] = (
            coords[0] + dx,
            coords[1] + dy,
            coords[2],
        )
    ...
    translated_center = (
        None if center is None else (center[0] + dx, center[1] + dy, center[2])
    )

...
def _add_action_row(
    layout: QVBoxLayout, *, accept_label: str
...
def prompt_zoom_percent(
    ...
    up_btn.clicked.connect(
        lambda: spin.setValue(min(maximum, spin.value() + spin.singleStep()))
    )
    down_btn.clicked.connect(
        lambda: spin.setValue(max(minimum, spin.value() - spin.singleStep()))
    )

...
class _FrozenValue:
    ...
    def capture(cls, value: object) -> _FrozenValue:
        ...
        if isinstance(value, tuple):
            return cls(
                "tuple",
                value_type,
                children=tuple(cls.capture(item) for item in tuple.__iter__(value)),
            )
        ...
        if isinstance(value, HistoryCommand):
            ...
            return cls(
                ...
                identity=value,
                children=tuple(
                    (name, cls.capture(item)) for name, item in dict.items(namespace)
                ),
            )
    ...
    def verify(self, actual: Any, *, path: str) -> None:  # noqa: C901
        ...
        if self.kind == "tuple":
            ...
            if actual is not self.identity:
                raise RuntimeError(f"history command container {path} changed identity")
            ...
            if actual is not self.identity:
                raise RuntimeError(f"history command structure {path} changed identity")

...
class CanvasAtomMutationService:
    ...
    def add_atom(self, element: str, x: float, y: float) -> int:
        ...
        if element.upper() == "C":
            self.canvas.services.atom_label_service.ensure_carbon_dot(atom_id)
        else:
    ...
    def remove_atom_only(self, atom_id: int, remove_marks: bool = True) -> None:
        ...
        if remove_marks:
            self.canvas.services.canvas_mark_scene_service.remove_marks_for_atom(
                atom_id
            )
        self.canvas.model.pop_atom(atom_id)
    ...
    def restore_atom_from_state(self, atom_id: int, state: dict) -> None:
        ...
        self.canvas.model.set_atom(atom_id, atom)
        self.canvas.model.set_atom_annotation(atom_id, state.get("annotation"))
        self.graph_service.ensure_atom_neighbors(atom_id)
        self.graph_service.ensure_atom_bond_ids(atom_id)
        self.canvas.model.ensure_next_atom_id_after(atom_id)
        self._clear_atom_graphics(atom_id)
    ...
    def apply_atom_color(self, atom_id: int, color: str | QColor) -> None:
        ...
        label_item = self.canvas.runtime_state.atom_graphics_state.atom_items.get(
            atom_id
        )
        if label_item is not None:
            label_item.setDefaultTextColor(color_value)
        dot_item = self.canvas.runtime_state.atom_graphics_state.atom_dots.get(atom_id)

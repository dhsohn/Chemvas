...
class MainWindowToolIconRenderer:
    ...
    def _alpha_color(self, color: str | None, alpha: float) -> QColor:
        base = (
            self._icon_pen(self._stroke_thin).color()
            if color is None
            else QColor(color)
        )
    ...
    def _draw_glow_dot(
        self, painter, center: QPointF, radius: float, *, alpha: float = 0.82
    ) -> None:
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(
            QBrush(self._alpha_color(self._pale_fill_color, min(1.0, alpha * 0.62)))
        )
    ...
    def draw_ts_bracket(self, painter) -> None:
        ...
        self._draw_square_bracket_icon(
            painter, QRectF(18.0, 6.0, 8.0, 18.0), left=False
        )
    ...
    def _draw_square_bracket_icon(self, painter, rect: QRectF, *, left: bool) -> None:
        ...
        if left:
            ...
            painter.drawLine(
                QPointF(x, rect.bottom()), QPointF(x + hook, rect.bottom())
            )
            return
        x = rect.right()
        painter.drawLine(QPointF(x - hook, rect.top()), QPointF(x, rect.top()))
    ...
    def _draw_parenthesis_icon(self, painter, rect: QRectF, *, left: bool) -> None:
        ...
        if left:
            ...
            path.cubicTo(
                outer_x, top + control, outer_x, middle - control, outer_x, middle
            )
            path.cubicTo(
    ...
    def _draw_dagger_icon(self, painter, symbol: str) -> None:
        ...
        painter.drawText(
            QRectF(6.0, 4.5, 18.0, 21.0), Qt.AlignmentFlag.AlignCenter, symbol
        )

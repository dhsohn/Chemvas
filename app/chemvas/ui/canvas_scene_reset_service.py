...
from chemvas.ui.history_push_failure_recovery import RecordingHistoryPolicySnapshot
...
from chemvas.ui.spatial_index_state import mark_spatial_index_dirty_for
...
_RESET_OWNED_RUNTIME_FIELDS: dict[str, tuple[str, ...] | None] = {
    ...
    # HistoryAuthoritySnapshot/RecordingHistoryPolicySnapshot pair.
    "history_state": (),
    "history_service": (),
    "graph_state": None,
    "group_state": None,
...
class _RawSceneMembershipRoot:
    ...
    def capture(cls, scene: object) -> _RawSceneMembershipRoot | None:
        ...
        candidates: list[tuple[dict | None, MemberDescriptorType | None, list]] = []
        if namespace is not None:
...
class _ResetServiceAliasSnapshot:
    ...
    def capture(
        ...
        roots = {name: state for name, present, state in owned.state_roots if present}
    ...
    def verify(self) -> None:
        for name, expected in self.aliases:
            if _raw_attribute(self.service, name) is not expected:
                raise RuntimeError(f"scene reset changed service alias {name!r}")
...
class _RawHistoryAuthoritySnapshot:
    ...
    def matches_snapshot(
        self,
        snapshot: HistoryAuthoritySnapshot | None,
        ...
        captured_policy = () if policy_snapshot is None else policy_snapshot.ports
        ...
        return len(captured_by_name) == sum(
            1 for _name, storage_name, _value in self.policy if storage_name is not None
        )
...
class CanvasSceneResetService:
    ...
    def _capture_preflight_recovery_ports(
        ...
        if clear is None:
            ...
            def unavailable_static_clear() -> None:
                raise RuntimeError("scene has no callback-free clear port for recovery")
    ...
    def _capture_pre_reset_authority(
        ...
        *,
        ...
        if services is _MISSING_ATTRIBUTE:
            ...
            services = None if services_value is _MISSING_ATTRIBUTE else services_value
    ...
    def _restore_non_qt_scene_membership(
        ...
        if ports.raw_items_root is not None:
            try:
                ...
                return not CanvasSceneResetService._scene_membership_was_changed(ports)
            except BaseException:
                return False
    ...
    def _clear_uncaptured_history_stacks(
        ...
        A hostile ``state`` descriptor can fail before ``HistoryAuthoritySnapshot``
        ...
        *,
        ...
        if not destructive_or_unknown:
            try:
                ...
                if not destructive_or_unknown and raw_history_snapshot is not None:
    ...
    def clear_scene(self) -> None:  # noqa: C901
        ...
        raw_history_snapshot = _RawHistoryAuthoritySnapshot.capture(history_service)
        empty_model = MoleculeModel()
        ...
        def verify_pre_scene_authorities() -> None:
            ...
            if services is not None:
                ...
                if current_history_present is not history_present or (
                    ...
                    raise RuntimeError("scene getter changed the history-service root")

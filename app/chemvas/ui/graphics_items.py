...
class AtomLabelItem(NoSelectTextItem):
    ...
    geometry sourced from :mod:`chemvas.features.annotations`.
    """

    def __init__(
        self, *args, hit_padding: float = 0.0, hit_radius: float | None = None
    ) -> None:
        super().__init__(*args)
        self._hit_padding = max(0.0, float(hit_padding))
    ...
    def set_stack_anchor(
        self, element: str | None, *, hydrogens_below: bool = True
    ) -> None:
    ...
    def _measure_context(
        self,
    ) -> tuple[Callable[[str, float], float], float, float, float]:
        font = self.font()
    ...
    def _relayout(self) -> None:
        ...
        if self._stack is not None:
            ...
            self._stack_element_rect = QRectF(
                margin + box_x, margin + box_y, box_w, box_h
            )
            return

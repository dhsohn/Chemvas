...
class BondGeometryUpdateService:
    ...
    def _update_hash_geometry(self, bond, items, a, b) -> None:
        ...
        segments = self.renderer.hash_segments(
            a.x, a.y, b.x, b.y, count, bond.a, bond.b
        )
    ...
    def _update_dotted_geometry(self, bond, items, a, b) -> None:
        ...
        if len(items) == 1 and isinstance(items[0], QGraphicsPathItem):
            items[0].setPath(
                self.renderer.dotted_bond_path(
                    start_x, start_y, end_x, end_y, bond.a, bond.b
                )
            )
    ...
    def _update_dotted_double_geometry(self, bond, items, a, b) -> None:
        base_style, outer_seg, inner_seg = self._dotted_double_segments(
            bond, items, a, b
        )
        if len(items) < 2:
            return
        ...
        if outer_dotted:
            if isinstance(items[0], QGraphicsPathItem):
                items[0].setPath(
                    self.renderer.dotted_bond_path(*outer_seg, bond.a, bond.b)
                )
    ...
    def _update_bold_multi_geometry(
        self, bond, items, a, b, bold_outward: bool
    ) -> None:
        if bond.order == 2:
    ...
    def _update_bold_single_geometry(
        self, bond, items, a, b, bold_outward: bool
    ) -> None:
    ...
    def _update_plain_double_geometry(self, bond, items, a, b) -> None:
        ...
        if (
            len(items) >= 2
            and isinstance(items[0], QGraphicsLineItem)
            and isinstance(items[1], QGraphicsLineItem)
        ):
            items[0].setLine(*outer_seg)
            items[1].setLine(*inner_seg)

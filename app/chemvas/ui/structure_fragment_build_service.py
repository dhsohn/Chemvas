...
class StructureFragmentBuildService:
    ...
    def add_regular_ring_template(
        self, n: int, actions: StructureFragmentBuildActions
    ) -> None:
    ...
    def add_fused_benzenes(
        self, count: int, mode: str, actions: StructureFragmentBuildActions
    ) -> None:
        center = actions.viewport_center()
        ...
        for ring_center in fused_benzene_centers(center, step, count, mode):
            ...
            bond_orders = [
                order for _, _, order in alternating_ring_bond_specs(range(len(points)))
            ]
            actions.add_ring_from_points(points, merge=merge, bond_orders=bond_orders)

    def add_crown_ether(
        self, atoms: int, oxygens: int, actions: StructureFragmentBuildActions
    ) -> None:
        center = actions.viewport_center()
        points = actions.ring_points(
            center, atoms, radius=bond_length_px_for(self.canvas) * 1.4
        )
        actions.add_ring_from_points(
            points, elements=crown_ether_elements(atoms, oxygens)
        )
    ...
    def add_fused_heterocycle_template(
        ...
        *,
        ...
        ring_result = actions.regular_ring_points_for_bond(
            ring_size, bond_id, center_hint
        )
    ...
    def add_phenyl(self, actions: StructureFragmentBuildActions) -> None:
        def _build() -> None:
            ...
            attach = QPointF(
                center.x() - bond_length_px_for(self.canvas) * 2.0, center.y()
            )
...
def _benzene_bond_orders(*, first_order: int = 2) -> list[int]:
    return [
        order
        for _, _, order in alternating_ring_bond_specs(
            range(6), first_order=first_order
        )
    ]


def _fused_heterocycle_bond_orders(
    ring_size: int, elements: list[str]
) -> list[int] | None:
    if ring_size == 6:

...
class CanvasGraphService:
    def __init__(self, canvas: CanvasView, graph_state: CanvasGraphState | None = None) -> None:
        self.canvas = canvas
    ...
    def rebuild_bond_adjacency(self) -> None:
        self.graph.atom_neighbors, self.graph.atom_bond_ids = build_bond_adjacency_index(
    ...
    def component_without_bond(self, start_atom_id: int, skip_bond_id: int) -> set[int]:
        ...
        if skip_bond is not None:
            shared = self.graph.atom_bond_ids.get(skip_bond.a, set()) & self.graph.atom_bond_ids.get(
                skip_bond.b, set()
            )

...
from ui.structure_geometry_access import default_bond_endpoint_for
...
class BondTool(Tool):
    ...
    def _clear_preview_items(self) -> None:
        if not self._preview_items:
            ...
            return
        self._preview_items = clear_bond_preview_items_for(self.canvas, self._preview_items)
        self._preview_signature = None
    ...
    def _set_preview_items(self, start: QPointF, end: QPointF) -> None:
        ...
        if self._preview_items and self._preview_signature == signature:
            if update_bond_preview_items_for(
                ...
                return
        self._clear_preview_items()
        items = build_bond_preview_items_for(self.canvas, start, end, self._start_atom_id, None)
        if not items:
            return
    ...
    def on_mouse_release(self, event) -> bool:
        ...
        if dist < bond_length_px_for(self.canvas) * 0.1:
            end_pos = default_bond_endpoint_for(self.canvas, self._start_pos, self._start_atom_id)
        self._clear_preview_items()
        settings = tool_settings_state_for(self.canvas)
    ...
    def _snap_to_atom(self, pos, ignore_start: bool = False):
        ...
        bond_id = None
        if atom_id is None:
            bond_id = self.context.find_bond_near(pos, bond_length_px_for(self.canvas) * 0.2)

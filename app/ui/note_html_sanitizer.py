...
SKIP_NOTE_HTML_CONTENT_TAGS = frozenset(
    ("embed", "head", "iframe", "math", "object", "script", "style", "svg", "template", "title")
)
SAFE_NOTE_HTML_ATTRS_BY_TAG = {tag: frozenset(("style",)) for tag in SAFE_NOTE_HTML_TAGS - VOID_NOTE_HTML_TAGS}
...
def _is_safe_style_property_value(property_name: str, value: str) -> bool:
    ...
    if property_name == "font-size":
        return lower_value in _CSS_FONT_SIZE_KEYWORDS or _CSS_LENGTH_RE.fullmatch(lower_value) is not None
    ...
    if property_name in {"margin-bottom", "margin-left", "margin-right", "margin-top", "text-indent"}:
...
def _is_safe_color_value(value: str) -> bool:
    ...
    if _CSS_COLOR_RE.fullmatch(value) is not None or lower_value in _CSS_NAMED_COLORS:
        return True
    return _CSS_RGB_COLOR_RE.fullmatch(lower_value) is not None and _is_safe_rgb_color_value(lower_value)
...
def _sanitized_list_type_value(tag: str, value: str) -> str | None:
    ...
    if tag == "ul" and lower_value in {"circle", "disc", "square"}:
        return lower_value
    if tag == "li" and (value in {"1", "A", "I", "a", "i"} or lower_value in {"circle", "disc", "square"}):

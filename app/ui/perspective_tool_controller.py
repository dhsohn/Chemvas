...
class PerspectiveToolController:
    ...
    def _begin_selection_3d_rotation(self, *, axis_hint: int | None = None, press_pos=None) -> bool:
        return bool(
    ...
    def begin_selection_rotation(self, event) -> bool:
        ...
        preferred_item = self.selection_controller.preferred_structure_item_at_scene_pos(press_pos)
        if not self.selection_controller.selection_hit_test(press_pos, snapshot=None):
            ...
            if item is None or not self.selection_controller.select_structure_for_item(item):
                return False
            preferred_item = self.selection_controller.preferred_structure_item_at_scene_pos(press_pos)
        axis_hint = self.axis_hint_for_item(preferred_item)
        return self._begin_selection_3d_rotation(axis_hint=axis_hint, press_pos=press_pos)

...
def structure_selection_targets_for_item(
    ...
    *,
    ...
    if item is None:
        return []
    atom_item_for_id = atom_item_for_id or (lambda atom_id: visible_atom_item_for(canvas, atom_id))
    kind = item.data(0)
    if kind == "atom":
        atom_id = item.data(1)
    ...
    if kind == "bond":
        ...
        if not isinstance(bond_id, int):
            return []
        return [bond_item for bond_item in bond_items_for_id(canvas, bond_id) if bond_item is not None]

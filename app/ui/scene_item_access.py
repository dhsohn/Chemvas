...
def scene_item_controller(canvas):
    return canvas._scene_item_controller


def restore_ring_from_state(canvas, ring_state: dict):
    return scene_item_controller(canvas)._restore_ring_from_state(ring_state)


def restore_note_from_state(canvas, note_state: dict):
    return scene_item_controller(canvas)._restore_note_from_state(note_state)


def restore_mark_from_state(canvas, mark_state: dict) -> None:
    scene_item_controller(canvas)._restore_mark_from_state(mark_state)


def restore_arrow_from_state(canvas, arrow_state: dict):
    return scene_item_controller(canvas)._restore_arrow_from_state(arrow_state)


def restore_ts_bracket_from_state(canvas, ts_bracket_state: dict):
    return scene_item_controller(canvas)._restore_ts_bracket_from_state(ts_bracket_state)


def restore_orbital_from_state(canvas, orbital_state: dict):
    return scene_item_controller(canvas)._restore_orbital_from_state(orbital_state)

...
class ArrowTool(PreviewDragTool):
    ...
    def _arrow_type(self) -> str:
        return self.mode if self.mode != "auto" else tool_settings_state_for(self.canvas).active_arrow_type

    def _build_preview(self, current_pos):
        return preview_arrow_for(self.canvas, self._start_pos, current_pos, self._arrow_type())

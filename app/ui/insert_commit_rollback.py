...
from ui.history_canvas_access import restore_history_transaction_for_history
...
class SmilesInputRestoreAuthority:
    ...
    def capture(cls, canvas: object) -> SmilesInputRestoreAuthority:
        ...
        callback_free_root = (
            static_owner_value is state
            or isinstance(static_owner_value, MemberDescriptorType)
        )
    ...
    def restore(self, target: str | None) -> HistoryTransactionRestoreResult:
        ...
        for attempt in range(2):
            ...
            try:
                if self.state_getter() is not self.state:
                    raise RuntimeError(
                        "SMILES rollback did not restore state identity"
                    )
...
def rollback_insert_mutation(
    ...
    *,
    ...
    if not rollback_errors:
        return HistoryTransactionRestoreResult(authoritative=authoritative)

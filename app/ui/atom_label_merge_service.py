...
from ui.scene_item_state import atom_state_dict_for, bond_state_dict
...
class AtomLabelMergeService:
    ...
    def merge_overlapping_atoms(self, atom_id: int) -> tuple[list[int], dict]:
        ...
        if not merge_ids:
            return [], {}
        merge_info = {
            "atom_states": {mid: atom_state_dict_for(self.canvas, mid) for mid in merge_ids},
            "bond_before_states": {},
            "deleted_bond_ids": [],
        }
    ...
    def _capture_bond_states_touching_merged_atoms(self, merge_ids: list[int], merge_info: dict) -> None:

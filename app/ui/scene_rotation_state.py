    ...
    *,
    ...
    if kind == "note":
        ...
        if rect.isValid():
            ...
            after_state["x"] = before_state.get("x", 0.0) + rotated_center.x() - rect.center().x()
            after_state["y"] = before_state.get("y", 0.0) + rotated_center.y() - rect.center().y()
        else:
    ...
    if kind == "orbital":
        ...
        after_state["rotation"] = (float(before_state.get("rotation", 0.0)) + angle_degrees) % 360.0
        return after_state
    if kind in {"ts_bracket", "shape"}:
        ...
        rotated_rect.moveCenter(rotated_point(state_rect.center(), center, angle_radians))

...
class CallbackFreeHistoryBaseline:
    ...
    def capture(
        ...
        *,
        ...
        for candidate in candidates:
            _key, state, _namespace, _history_key, _redo_key, history, redo = (
                candidate
            )
        ...
        for public_name in ("enabled", "limit"):
            ...
            if roots:
                ...
                policy_values.append(
                    (public_name, None, _MISSING_RAW_HISTORY_ROOT)
                )
        raw_baseline = cls(
    ...
    def _verify_expected_items(
        ...
        *,
        ...
        self.aliases.verify()
        if (
            dict.get(self.service_namespace, self.service_state_key)
            is not self.state
        ):
            raise RuntimeError("raw history service state identity changed")
        if (
            dict.get(self.state_namespace, self.state_history_key)
            is not self.history
        ):
            raise RuntimeError("raw history-list identity changed")
        if (
            dict.get(self.state_namespace, self.state_redo_key)
            is not self.redo_stack
        ):
        ...
        for public_name, storage_name, expected in self.policy_values:
            ...
            if not exact:
                raise RuntimeError(
                    f"raw history policy {public_name!r} changed"
                )
        self.aliases.verify()
...
def _capture_recording_history_snapshot(
    ...
    if (
        ...
        return HistoryStackSnapshot.capture(history)
    ...
    try:
        ...
        notify_change = None
    raw_baseline.verify()
    return HistoryStackSnapshot.capture(
    ...
    if accepted:
        ...
        if policy_snapshot is not None:
            limit = next(
                (
                    port.value
                    for port in policy_snapshot.ports
                    if port.name == "limit"
                ),
                None,
            )
...
class CanvasHistoryRecordingService:
    ...
    def _push_history(self, command: HistoryCommand) -> None:
        ...
        try:
            ...
            command_snapshot.verify()
            if raw_history_baseline is not None:
                raw_history_baseline.verify_published_commands(
                    ((command, accepted),)
                )
        except BaseException as error:

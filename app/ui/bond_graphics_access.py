...
def line_normal_components(x1: float, y1: float, x2: float, y2: float) -> tuple[float, float, float]:
    dx = x2 - x1
    dy = y2 - y1
...
def line_normal_for(canvas, x1: float, y1: float, x2: float, y2: float, target=None) -> tuple[float, float]:

...
from ui.canvas_model_access import bond_for_id, bonds_for
from ui.graph_algorithms import edge_has_reachable_alternative_path
...
def bond_in_cycle_for(canvas, bond_id: int) -> bool:
    ...
    if bond is None:
        ...
        return False
    shared = graph.atom_bond_ids.get(bond.a, set()) & graph.atom_bond_ids.get(bond.b, set())
...
def bond_is_planar_fragment_edge_for(canvas, bond_id: int, *, bond_in_cycle=None) -> bool:
    if bond_in_cycle is None:
    ...
    if bond.order > 1 or bond_in_cycle(bond_id):
        return True
    return atom_in_planar_system_for(canvas, bond.a, bond_in_cycle=bond_in_cycle) and atom_in_planar_system_for(
        canvas,
        bond.b,
...
def planar_fragment_components_for(canvas, atom_ids: set[int], *, bond_in_cycle=None) -> list[set[int]]:
    ...
    for bond_id, bond in enumerate(bonds_for(canvas)):
        ...
        if bond.a not in atom_ids or bond.b not in atom_ids:
            continue
        if not bond_is_planar_fragment_edge_for(canvas, bond_id, bond_in_cycle=bond_in_cycle):
            continue
...
def flatten_planar_fragments_for(
    ...
    *,
    ...
    for fragment in planar_fragment_components_for(canvas, atom_ids, bond_in_cycle=bond_in_cycle):

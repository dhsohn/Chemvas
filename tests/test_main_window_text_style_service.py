...
class MainWindowTextStyleServiceTest(unittest.TestCase):
    ...
    def test_color_actions_apply_only_valid_colors(self) -> None:
        valid = QColor("#112233")
        invalid = QColor()

        self.service.set_text_color(self.window, get_color=lambda **kwargs: invalid)
        self.service.set_note_box_color(self.window, get_color=lambda **kwargs: valid)
        self.service.set_note_border_color(self.window, get_color=lambda **kwargs: valid)

        self.window.canvas.set_text_color.assert_not_called()
        self.window.canvas.set_note_box_color.assert_called_once_with(valid)
        self.window.canvas.set_note_border_color.assert_called_once_with(valid)

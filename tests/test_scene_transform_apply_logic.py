...
class SceneTransformApplyLogicTest(unittest.TestCase):
    ...
    def test_apply_component_flip_transform_adds_atom_and_scene_item_commands_when_changed(
        self,
    ) -> None:
        ...
        commands = apply_component_flip_transform(
            ...
            set_atom_positions=lambda positions, update_selection=False: (
                set_positions_calls.append((dict(positions), update_selection))
            ),
            apply_scene_item_state=lambda item, state: applied_states.append(
                (item, dict(state))
            ),
        )
    ...
    def test_apply_component_flip_transform_skips_noop_atom_and_item_changes(
        self,
    ) -> None:
    ...
    def test_apply_standalone_flip_transform_returns_command_only_for_changed_state(
        self,
    ) -> None:

...
class CanvasViewSelectionClipboardWrappersTest(unittest.TestCase):
    ...
    def test_selected_items_for_transform_filters_excluded_items_and_deduplicates_notes(
        self,
    ) -> None:
    ...
    def test_selected_scene_notes_and_items_filter_scene_membership_and_excluded_kinds(
        self,
    ) -> None:
        scene = SimpleNamespace()
    ...
    def test_append_unique_scene_item_skips_duplicates_and_excluded_kinds(self) -> None:
        ...
        self.assertTrue(
            append_unique_scene_item(items, seen, atom, excluded_kinds={"handle"})
        )
        self.assertFalse(
            append_unique_scene_item(items, seen, atom, excluded_kinds={"handle"})
        )
        self.assertFalse(
            append_unique_scene_item(items, seen, handle, excluded_kinds={"handle"})
        )
        self.assertEqual(items, [atom])

    def test_selected_id_helpers_cover_atom_bond_ring_polygon_and_mark_fallback_paths(
        self,
    ) -> None:
        polygon = QPolygonF(
            [
        ...
        append_selected_item_ids(
            view, atom_ids, bond_ids, _FakeItem("ring", data2=[2, "bad", 99])
        )
        append_selected_item_ids(
            view, atom_ids, bond_ids, _FakeItem("ring", polygon=polygon)
        )
    ...
    def test_selection_items_for_copy_includes_selected_bond_endpoint_atom_graphics(
        self,
    ) -> None:
    ...
    def test_helper_tails_cover_invalid_metadata_bounds_and_mark_kind_filters(
        self,
    ) -> None:
        view = SimpleNamespace(
        ...
        append_selected_item_ids(
            view, atom_ids, bond_ids, _FakeItem("atom", data1="bad")
        )
        append_selected_item_ids(
            view, atom_ids, bond_ids, _FakeItem("bond", data1="bad")
        )
        append_selected_item_ids(
            view,
            atom_ids,
            bond_ids,
            _FakeItem("ring", data2="bad", polygon=QPolygonF()),
        )
        self.assertEqual(atom_ids, set())
        self.assertEqual(bond_ids, set())

    def test_selected_atom_ids_for_transform_includes_bond_endpoints_and_skips_invalid_bonds(
        self,
    ) -> None:
        scene = SimpleNamespace(
            selectedItems=lambda: [

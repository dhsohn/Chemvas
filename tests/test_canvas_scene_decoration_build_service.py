...
@unittest.skipUnless(
    QApplication is not None and CanvasSceneDecorationBuildService is not None and CanvasView is not None,
...
class CanvasSceneDecorationBuildServiceTest(unittest.TestCase):
    ...
    def test_arrow_helpers_delegate_to_arrow_build_service(self) -> None:
        ...
        arrow_service.build_single_head_arrow.assert_called_once_with(QPointF(1.0, 1.0), QPointF(2.0, 2.0))
        arrow_service.build_double_head_arrow.assert_called_once_with(QPointF(1.0, 1.0), QPointF(2.0, 2.0))
        arrow_service.build_dotted_arrow.assert_called_once_with(QPointF(1.0, 1.0), QPointF(2.0, 2.0))
        arrow_service.build_curved_arrow.assert_called_once_with(QPointF(1.0, 1.0), QPointF(2.0, 2.0), True)
        arrow_service.build_inhibition_arrow.assert_called_once_with(QPointF(1.0, 1.0), QPointF(2.0, 2.0))
        arrow_service.build_equilibrium_item.assert_called_once_with(QPointF(1.0, 1.0), QPointF(2.0, 2.0))
        arrow_service.add_arrow_head.assert_called_once_with(path, QPointF(3.0, 3.0), QPointF(4.0, 4.0), False)
        arrow_service.arrow_pen.assert_called_once_with(dotted=True)

    def test_preview_helpers_add_items_to_scene(self) -> None:
        arrow = self.service.preview_arrow(QPointF(0.0, 0.0), QPointF(12.0, 0.0), "reaction")
    ...
    def test_ts_bracket_helpers_normalize_rect_and_attach_metadata(self) -> None:
        rect = self.service.ts_bracket_rect_from_points(QPointF(4.0, 5.0), QPointF(5.0, 6.0))
        large_rect = self.service.ts_bracket_rect_from_points(QPointF(-30.0, -10.0), QPointF(30.0, 18.0))
    ...
    def test_build_orbital_items_covers_remaining_orbital_variants_and_unknown_kind(self) -> None:
        ...
        bonding_items = self.service.build_orbital_items(QPointF(0.0, 0.0), "mo_bonding")
        ...
        self.assertTrue(all(item.brush().style() != Qt.BrushStyle.NoBrush for item in sp2_items))
        centers = [item.rect().center() for item in sp2_items]
        self.assertEqual(len({(round(center.x(), 4), round(center.y(), 4)) for center in centers}), 3)
        self.assertTrue(any(center.x() > 0.0 for center in centers))
        self.assertTrue(any(center.x() < 0.0 and center.y() > 0.0 for center in centers))
        self.assertTrue(any(center.x() < 0.0 and center.y() < 0.0 for center in centers))
        self.assertEqual(len(sp3_items), 4)
        self.assertTrue(all(item.brush().style() != Qt.BrushStyle.NoBrush for item in sp3_items))

...
def test_template_geometry_resolver_service_builds_template_resolvers() -> None:
    ...
    with (
        ...
        assert list(resolvers.ring_points((1.0, 2.0), 6, 12.0)) == [
            (1.0, 2.0),
            (3.0, 4.0),
        ]
        assert list(resolvers.regular_ring_points_for_atom(6, 7) or []) == [
            (13.0, 14.0)
        ]
        assert list(resolvers.regular_ring_points_for_bond(6, 3, (4.0, 5.0)) or []) == [
            (5.0, 6.0)
        ]
        assert list(resolvers.chair_points((0.0, 0.0))) == [(7.0, 8.0)]
        assert list(resolvers.boat_points((0.0, 0.0))) == [(9.0, 10.0)]
        assert list(
            resolvers.template_points_for_bond([(0.0, 0.0)], 4, (2.0, 3.0)) or []
        ) == [(11.0, 12.0)]
...
def test_template_geometry_resolver_service_resolves_planned_insert_and_pair_points() -> (
    ...
    assert [
        (point.x(), point.y()) for point in service.points_from_pairs([(3.0, 4.0)])
    ] == [(3.0, 4.0)]

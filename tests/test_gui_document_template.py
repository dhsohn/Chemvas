...
class GuiDocumentAndTemplateTest(unittest.TestCase):
    ...
    def _template_handler(self, label: str):
        entries = dict(services_for_window(self.window).tool_routing_service.template_entries(self.window))
    ...
    def test_save_canvas_appends_extension_and_writes_document_payload(self) -> None:
        ...
        canvas_services_for(active_canvas_for_window(self.window)).note_controller.create_text_note(QPointF(60.0, 10.0), "Scheme")
        add_mark_for(active_canvas_for_window(self.window), QPointF(20.0, 20.0), kind="plus", atom_id=0, record=False)
        add_arrow_for(active_canvas_for_window(self.window), QPointF(-40.0, -20.0), QPointF(40.0, -20.0), "reaction")
    ...
    def test_load_canvas_restores_document_and_resets_history(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            add_benzene_ring_for(active_canvas_for_window(self.window), QPointF(0.0, 0.0))
            canvas_services_for(active_canvas_for_window(self.window)).note_controller.create_text_note(QPointF(75.0, 10.0), "Roundtrip")
            add_mark_for(active_canvas_for_window(self.window), QPointF(20.0, 20.0), kind="minus", atom_id=0, record=False)
            add_arrow_for(active_canvas_for_window(self.window), QPointF(-50.0, -10.0), QPointF(50.0, -10.0), "equilibrium")
            ...
            history_state_for(active_canvas_for_window(self.window)).redo_stack = ["dirty"]
            ...
            with patch.object(active_canvas_for_window(self.window).rdkit, "smiles_to_2d", return_value=preview_model):
                active_canvas_for_window(self.window).services.insert_controller.begin_smiles_insert("CC")
            self.assertTrue(insert_state_for(active_canvas_for_window(self.window)).smiles_active)
        ...
        self.assertEqual(last_smiles_input_for(active_canvas_for_window(self.window)), "NCCO")
        self.assertEqual(history_state_for(active_canvas_for_window(self.window)).history, [])
        self.assertEqual(history_state_for(active_canvas_for_window(self.window)).redo_stack, [])
        self.assertFalse(insert_state_for(active_canvas_for_window(self.window)).smiles_active)
        self.assertIsNone(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_model)
        self.assertIsNone(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_smiles)
        self.assertIsNone(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_center)
        self.assertEqual(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_items, [])

    def test_snapshot_restore_reapplies_saved_settings_before_recreating_items(self) -> None:
        saved_weight = 77
        geometry_controller = canvas_services_for(active_canvas_for_window(self.window)).geometry_controller
        tool_mode_controller = canvas_services_for(active_canvas_for_window(self.window)).tool_mode_controller
        ...
        canvas_services_for(active_canvas_for_window(self.window)).note_controller.create_text_note(QPointF(30.0, 15.0), "Styled")
        add_arrow_for(active_canvas_for_window(self.window), QPointF(-40.0, 0.0), QPointF(40.0, 0.0), "arrow")
        ...
        self.assertAlmostEqual(active_canvas_for_window(self.window).renderer.style.bond_length_px, 28.0)
        ...
        self.assertTrue(tool_settings_state_for(active_canvas_for_window(self.window)).orbital_phase_enabled)
    ...
    def test_snapshot_restore_preserves_atom_bound_mark_offsets(self) -> None:
        ...
        add_mark_for_atom_for(active_canvas_for_window(self.window), atom_id, QPointF(42.0, -20.0), kind="minus", record=False)
        ...
        restored_center = mark_center_for(active_canvas_for_window(self.window), restored_mark)
    ...
    def test_save_canvas_as_failure_warns_and_preserves_current_path(self) -> None:
        ...
        with (
            patch("ui.main_window_document_action_service.QFileDialog.getSaveFileName", return_value=("/tmp/renamed.chemvas", "")),
            patch.object(doc_service, "save_to_file", side_effect=OSError("disk full")) as save_mock,
    ...
    def test_export_xyz_appends_extension_and_updates_status_message(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            doc_service = active_canvas_for_window(self.window).services.canvas_document_session_service
            with (
                patch("ui.main_window_document_action_service.QFileDialog.getSaveFileName", return_value=(str(raw_path), "")),
                patch.object(doc_service, "export_xyz_async", side_effect=export_success) as export_mock,
            ):
                self.assertFalse(hasattr(active_canvas_for_window(self.window), "export_xyz_async"))
    ...
    def test_export_xyz_failure_warns_and_preserves_status_message(self) -> None:
        ...
        with (
            patch("ui.main_window_document_action_service.QFileDialog.getSaveFileName", return_value=("/tmp/output.xyz", "")),
            patch.object(
    ...
    def test_load_canvas_failure_warns_and_preserves_existing_scene(self) -> None:
        add_benzene_ring_for(active_canvas_for_window(self.window), QPointF(0.0, 0.0))
        canvas_services_for(active_canvas_for_window(self.window)).note_controller.create_text_note(QPointF(75.0, 10.0), "Keep me")
        set_last_smiles_input_for(active_canvas_for_window(self.window), "CCO")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch("ui.main_window_document_action_service.QFileDialog.getOpenFileName", return_value=(str(invalid_path), "")),
                patch("ui.main_window_document_action_service.QMessageBox.warning") as warning,
            ):
                self._document_actions().load_canvas(self.window)
        ...
        self.assertEqual(len(ring_items_for(active_canvas_for_window(self.window))), 1)
        self.assertEqual(len(note_items_for(active_canvas_for_window(self.window))), 1)
        self.assertEqual(last_smiles_input_for(active_canvas_for_window(self.window)), "CCO")
        self.assertEqual(history_state_for(active_canvas_for_window(self.window)).history, ["keep-history"])
        self.assertEqual(history_state_for(active_canvas_for_window(self.window)).redo_stack, ["keep-redo"])
    ...
    def test_regular_template_preview_reuses_existing_preview_items(self) -> None:
        ...
        self.assertEqual(len(insert_state_for(active_canvas_for_window(self.window)).template_preview_lines), 3)
        self.assertEqual(len(insert_state_for(active_canvas_for_window(self.window)).template_preview_dots), 3)
        preview_lines = list(insert_state_for(active_canvas_for_window(self.window)).template_preview_lines)
        ...
        self.assertEqual(len(insert_state_for(active_canvas_for_window(self.window)).template_preview_lines), 3)
        self.assertEqual(len(insert_state_for(active_canvas_for_window(self.window)).template_preview_dots), 3)
        self.assertIs(insert_state_for(active_canvas_for_window(self.window)).template_preview_lines[0], preview_lines[0])
        self.assertNotEqual(insert_state_for(active_canvas_for_window(self.window)).template_preview_lines[0].line(), first_line_before)
    ...
    def test_large_regular_template_preview_uses_requested_ring_size(self) -> None:
        for label, ring_size in (("Cycloheptane", 7), ("Cyclooctane", 8)):
            with self.subTest(label=label):
                ...
                self.assertTrue(insert_state_for(active_canvas_for_window(self.window)).template_active)
                self.assertEqual(insert_state_for(active_canvas_for_window(self.window)).template_ring_size, ring_size)
                ...
                self.assertEqual(len(insert_state_for(active_canvas_for_window(self.window)).template_preview_lines), ring_size)
                self.assertEqual(len(insert_state_for(active_canvas_for_window(self.window)).template_preview_dots), ring_size)
                active_canvas_for_window(self.window).services.insert_controller.cancel_template_insert()
    ...
    def test_regular_template_commit_on_bond_merges_existing_endpoints(self) -> None:
        ...
        self.assertEqual(len(insert_state_for(active_canvas_for_window(self.window)).template_preview_lines), 4)
    ...
    def test_chair_template_commit_on_ring_bond_places_new_atoms_outside_existing_ring(self) -> None:
        ...
        bond_id = active_canvas_for_window(self.window).services.canvas_graph_service.bond_id_between(ring_atom_ids[0], ring_atom_ids[1])
        ...
        self.assertEqual(len(insert_state_for(active_canvas_for_window(self.window)).template_preview_lines), 6)
    ...
    def test_escape_cancels_template_insert_and_prevents_commit(self) -> None:
        ...
        preview_items = list(insert_state_for(active_canvas_for_window(self.window)).template_preview_items)
        ...
        bond_count_before = sum(1 for bond in active_canvas_for_window(self.window).model.bonds if bond is not None)
        active_canvas_for_window(self.window).services.insert_controller.commit_template_insert(QPointF(25.0, 10.0))

        self.assertFalse(insert_state_for(active_canvas_for_window(self.window)).template_active)
        self.assertIsNone(insert_state_for(active_canvas_for_window(self.window)).template_ring_size)
        self.assertIsNone(insert_state_for(active_canvas_for_window(self.window)).template_ring_style)
        self.assertEqual(insert_state_for(active_canvas_for_window(self.window)).template_preview_items, [])
        self.assertEqual(insert_state_for(active_canvas_for_window(self.window)).template_preview_lines, [])
        self.assertEqual(insert_state_for(active_canvas_for_window(self.window)).template_preview_dots, [])
        self.assertEqual(len(active_canvas_for_window(self.window).model.atoms), atom_count_before)
        self.assertEqual(
            sum(1 for bond in active_canvas_for_window(self.window).model.bonds if bond is not None),
            bond_count_before,
        )
    ...
    def test_begin_smiles_insert_cancels_active_template_preview(self) -> None:
        ...
        with patch.object(active_canvas_for_window(self.window).rdkit, "smiles_to_2d", return_value=model):
            active_canvas_for_window(self.window).services.insert_controller.begin_smiles_insert("CC")
        ...
        self.assertTrue(all(item.scene() is None for item in preview_items))
        self.assertTrue(insert_state_for(active_canvas_for_window(self.window)).smiles_active)
        self.assertEqual(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_smiles, "CC")
        self.assertGreater(len(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_items), 0)
    ...
    def test_begin_template_insert_cancels_active_smiles_preview(self) -> None:
        ...
        self.assertTrue(insert_state_for(active_canvas_for_window(self.window)).smiles_active)
        smiles_preview_items = list(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_items)
    ...
    def test_smiles_preview_reuses_existing_preview_items(self) -> None:
        ...
        self.assertTrue(insert_state_for(active_canvas_for_window(self.window)).smiles_active)
        self._hover_scene_point(QPointF(-30.0, 0.0))
        self.assertEqual(len(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_bond_items[0]), 1)
        preview_line = insert_state_for(active_canvas_for_window(self.window)).smiles_preview_bond_items[0][0]
        preview_rect = insert_state_for(active_canvas_for_window(self.window)).smiles_preview_atom_items[left].rect()
        preview_atom = insert_state_for(active_canvas_for_window(self.window)).smiles_preview_atom_items[left]

        self._hover_scene_point(QPointF(30.0, 20.0))

        self.assertIs(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_bond_items[0][0], preview_line)
        self.assertIs(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_atom_items[left], preview_atom)
        self.assertNotEqual(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_atom_items[left].rect(), preview_rect)
    ...
    def test_clear_scene_resets_active_smiles_insert_state(self) -> None:
        ...
        self.assertTrue(insert_state_for(active_canvas_for_window(self.window)).smiles_active)
        self.assertIsNotNone(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_model)
        self.assertGreater(len(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_items), 0)
    ...
    def test_smiles_insert_commit_adds_atoms_bonds_and_clears_preview(self) -> None:
        ...
        with patch.object(active_canvas_for_window(self.window).rdkit, "smiles_to_2d", return_value=model):
            active_canvas_for_window(self.window).services.insert_controller.begin_smiles_insert("CN")

        self.assertTrue(insert_state_for(active_canvas_for_window(self.window)).smiles_active)
        preview_items = list(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_items)
        active_canvas_for_window(self.window).services.insert_controller.commit_smiles_insert(QPointF(40.0, 10.0))
        ...
        self.assertEqual(last_smiles_input_for(active_canvas_for_window(self.window)), "CN")
        self.assertEqual(insert_state_for(active_canvas_for_window(self.window)).smiles_preview_items, [])
        self.assertTrue(all(item.scene() is None for item in preview_items))
    ...
    def test_escape_cancels_smiles_insert_and_prevents_commit(self) -> None:
        ...
        self._press_key(Qt.Key.Key_Escape)
        active_canvas_for_window(self.window).services.insert_controller.commit_smiles_insert(QPointF(20.0, 0.0))
    ...
    def test_begin_smiles_insert_invalid_smiles_warns_without_entering_mode(self) -> None:
        ...
        with (
            patch.object(active_canvas_for_window(self.window).rdkit, "smiles_to_2d", return_value=None),
            patch("ui.insert_smiles_service.QMessageBox.warning") as warning,
        ):
            active_canvas_for_window(self.window).services.insert_controller.begin_smiles_insert("not-a-smiles")
    ...
    def test_canvas_export_xyz_uses_selected_structure_submodel(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(active_canvas_for_window(self.window).rdkit, "model_to_xyz_block", side_effect=_capture_export):
                canvas_services_for(active_canvas_for_window(self.window)).canvas_document_session_service.export_xyz(str(export_path))
    ...
    def test_canvas_export_xyz_passes_charge_and_radical_annotations(self) -> None:
        ...
        add_mark_for_atom_for(active_canvas_for_window(self.window), atom_id, QPointF(12.0, -12.0), kind="radical", record=False)
    ...
    def test_build_3d_conversion_payload_ignores_scene_only_items_in_mixed_selection(self) -> None:
        ...
        add_bond_graphics_for(active_canvas_for_window(self.window), 0)
        add_bond_graphics_for(active_canvas_for_window(self.window), 1)
        arrow = add_arrow_for(active_canvas_for_window(self.window), QPointF(-40.0, -20.0), QPointF(40.0, -20.0), "reaction")
        ts_bracket = add_ts_bracket_from_points_for(active_canvas_for_window(self.window), QPointF(-8.0, -28.0), QPointF(8.0, 28.0))
        note = canvas_services_for(active_canvas_for_window(self.window)).note_controller.create_text_note(QPointF(55.0, 10.0), "Scheme")
        ...
        export_model, atom_annotations = build_3d_conversion_payload_for(active_canvas_for_window(self.window))
    ...
    def test_build_3d_conversion_payload_uses_atom_bound_mark_selection(self) -> None:
        ...
        mark = add_mark_for_atom_for(active_canvas_for_window(self.window), left, QPointF(-12.0, -10.0), kind="plus", record=False)
        arrow = add_arrow_for(active_canvas_for_window(self.window), QPointF(-40.0, -20.0), QPointF(40.0, -20.0), "reaction")

...
class BenzenePreviewServiceTest(unittest.TestCase):
    def test_clear_preview_replaces_preview_pool(self) -> None:
        ...
        with mock.patch(
            "chemvas.ui.benzene_preview_service.clear_benzene_preview_for_canvas",
            return_value=[],
        ) as clear_preview:
    ...
    def test_render_preview_uses_structure_points_and_renderer(self) -> None:
        ...
        service = BenzenePreviewService(
            canvas, structure_build_service=structure_build_service
        )
        ...
        with (
            ...
            mock.patch(
                "chemvas.ui.benzene_preview_service.rebuild_benzene_preview_for_canvas",
                return_value=["new"],
            ) as rebuild_preview,
        ):
            service.render_preview(
                QPointF(5.0, 6.0), attach_atom_id=3, attach_bond_id=4
            )
        ...
        self.assertTrue(
            callable(rebuild_preview.call_args.kwargs["create_inner_bond_item"])
        )
        canvas.scene.assert_not_called()
    ...
    def test_create_inner_bond_item_returns_secondary_item(self) -> None:
        renderer = SimpleNamespace(
            draw_ring_double_bond=mock.Mock(return_value=["outer", "inner"])
        )
        canvas = SimpleNamespace(
        ...
        item = service._create_inner_bond_item(
            QPointF(1.0, 2.0), QPointF(3.0, 4.0), QPointF(5.0, 6.0)
        )

        self.assertEqual(item, "inner")
        renderer.draw_ring_double_bond.assert_called_once()

    def test_create_inner_bond_item_returns_none_when_secondary_item_is_missing(
        self,
    ) -> None:
        renderer = SimpleNamespace(
            draw_ring_double_bond=mock.Mock(return_value=["outer"])
        )
        canvas = SimpleNamespace(
            bond_renderer=renderer,
        )

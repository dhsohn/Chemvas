...
def test_update_action_availability_sets_history_actions(
    ...
    window = SimpleNamespace(
        ui_references=SimpleNamespace(
            undo_action=undo_action, redo_action=redo_action, grid_snap_action=None
        )
    )
    _patch_ports(
        monkeypatch,
...
def test_update_action_availability_handles_missing_canvas_and_actions(
    ...
    window = SimpleNamespace(
        ui_references=SimpleNamespace(
            undo_action=None, redo_action=None, grid_snap_action=None
        )
    )
    _patch_ports(
        monkeypatch,

...
class MainWindowToolStateServiceTest(unittest.TestCase):
    ...
    def test_set_tool_with_status_updates_canvas_status_and_optional_bond_reset(
        self,
    ) -> None:
        with (
            mock.patch.object(
            ...
            self.service.set_tool_with_status(
                self.window, "bond", reset_bond_style=False
            )
    ...
    def test_set_bond_style_routes_toolbar_labels_to_canvas(self) -> None:
        ...
        self.assertEqual(
            [call.args for call in set_bond_style.call_args_list],
            [("double", 2), ("single", 1)],
        )
    ...
    def test_set_arrow_and_orbital_variants_route_mapped_values(self) -> None:
        ...
        self.assertEqual(
            [call.args for call in set_arrow_type.call_args_list],
            [("curved_double",), ("reaction",)],
        )
        self.assertEqual(
            [call.args for call in set_orbital_type.call_args_list],
            [("mo_antibonding",), ("s",)],
        )
        self.assertEqual(
            [call.args for call in set_orbital_phase_enabled.call_args_list],
            [(True,), (False,)],
        )

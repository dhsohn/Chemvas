...
def test_ring_fill_opens_as_safe_selection_command_and_preserves_fill_workflow(
    ...
    action = window.ui_references.tool_action_for_key("ring_fill")
    _click(canvas, QPointF())
    assert canvas.services.canvas_document_session_service.snapshot_state() == before
    history.verify_stack_snapshot(stacks)
    assert not action.isCheckable()
    assert active_tool_name_for_window(window) == "select"
    assert window.ui_references.tool_action_for_key("select").isChecked()
    assert (
        window.services.status_service.active_tool_status_text(window) == "Tool: Select"
    )
...
def test_note_guard_does_not_lift_background_image_above_a_bond(drawing):
    ...
    _window, canvas = drawing
    _load(canvas)
    image = canvas.services.scene_item_controller.create_scene_item_from_state(
        image_state_from_bytes(image_bytes(), x=-30, y=-10, width=60)
    )
...
def test_edit_handle_above_caption_keeps_priority(drawing):
    ...
    arrow = canvas.services.scene_decoration_service.add_arrow(
        QPointF(-20, 0), QPointF(20, 0), "arrow"
    )
    note = canvas.services.note_controller.create_text_note(QPointF(-30, -6), "cat.")

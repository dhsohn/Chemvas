...
from ui.graph_rotation_policy import (
...
def test_preferred_rotation_side_policy_uses_coverage_distance_and_deterministic_fallback() -> None:
    bond = SimpleNamespace(a=1, b=2)
...
def test_rotatable_axis_policy_preserves_single_bond_leaf_boundary_and_candidate_rules() -> None:
    bonds = [
    ...
    assert rotatable_axis_from_selection_policy(
        ...
        rotation_side_for_bond=lambda bond_id, *args, **kwargs: {1, 4} if bond_id == 0 else None,
    ) == (0, {1, 4})
    assert rotatable_axis_from_selection_policy(
        {1},
        set(),
...
def test_axis_from_rotation_hint_policy_validates_rotatable_component_and_side() -> None:

...
import ui.selection_service_bundle as selection_service_bundle
...
def test_build_selection_services_injects_selection_collaborators(monkeypatch) -> None:
    for class_name in SERVICE_CLASS_NAMES:
        monkeypatch.setattr(selection_service_bundle, class_name, _stub_service_class(class_name))
    ...
    assert selection_preference_service.kwargs["hit_testing_service"] is hit_testing_service
    assert selection_preference_service.kwargs["structure_service"] is selection_structure_service
    assert selection_controller.kwargs["outline_service"].service_name == "SelectionOutlineService"
    assert selection_controller.kwargs["outline_service"].kwargs["graph_service"] is graph_service
    assert (
        selection_controller.kwargs["outline_service"].kwargs["active_tool_name_provider"]
        is active_tool_name_provider
    )
    assert selection_controller.kwargs["note_service"].service_name == "SelectionNoteService"
    assert selection_hit_test_service.service_name == "SelectionHitTestService"
    assert selection_hit_test_service.kwargs["hit_testing_service"] is hit_testing_service
    assert selection_hit_test_service.kwargs["structure_service"] is selection_structure_service
    assert selection_hit_test_service.kwargs["graph_service"] is graph_service


def test_build_selection_services_reuses_supplied_hit_testing_service(monkeypatch) -> None:
    for class_name in SERVICE_CLASS_NAMES:
        monkeypatch.setattr(selection_service_bundle, class_name, _stub_service_class(class_name))
    ...
    assert services.selection_controller.kwargs["hit_testing_service"] is supplied_hit_testing_service
    assert services.selection_controller.kwargs["preference_service"].kwargs["hit_testing_service"] is supplied_hit_testing_service
    assert services.selection_controller.kwargs["hit_test_service"].kwargs["hit_testing_service"] is supplied_hit_testing_service

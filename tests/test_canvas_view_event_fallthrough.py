...
class CanvasViewEventFallthroughTest(unittest.TestCase):
    ...
    def test_mouse_press_event_right_click_and_tool_false_fall_through_to_super(
        self,
    ) -> None:
        with mock.patch.object(
            ...
            CanvasView.mousePressEvent(
                tool_view, _FakeEvent(button=Qt.MouseButton.LeftButton)
            )
    ...
    def test_mouse_press_preserves_hover_until_non_perspective_tool_handles_target(
        self,
    ) -> None:
        tool = SimpleNamespace(
            name="bond",
    ...
    def test_mouse_press_event_right_click_on_double_bond_shows_variant_menu(
        self,
    ) -> None:
        ...
        handled = controller._show_double_bond_context_menu(
            _FakeEvent(
                button=Qt.MouseButton.RightButton, global_pos=QPointF(30.0, 40.0)
            ),
        ...
        self.assertEqual(
            [action.label for action in menu.actions], ["Inward", "Centered", "Outward"]
        )
        self.assertEqual(
            [action.checked for action in menu.actions], [False, True, False]
        )
        self.assertEqual(menu.exec_pos, QPointF(30.0, 40.0).toPoint())

        menu.actions[2].trigger()

        scene_transform_controller.apply_bond_style.assert_called_once_with(
            0, "double_outer", 2
        )
    ...
    def test_mouse_double_click_event_routes_non_select_tools_and_select_tool_falls_through(
        self,
    ) -> None:
        with mock.patch.object(
            QGraphicsView,
            ...
            tool = SimpleNamespace(
                name="bond", on_mouse_press=mock.Mock(return_value=True)
            )
            tool_view = self._new_view(tool_active=tool)

            CanvasView.mouseDoubleClickEvent(
                tool_view, _FakeEvent(button=Qt.MouseButton.LeftButton)
            )
            ...
            CanvasView.mouseDoubleClickEvent(
                select_view, _FakeEvent(button=Qt.MouseButton.LeftButton)
            )
    ...
    def test_mouse_move_event_fallthrough_calls_super_for_hover_and_drag_paths(
        ...
        with mock.patch.object(
            QGraphicsView, "mouseMoveEvent", new=mock.Mock(return_value=None)
        ) as base_move:
    ...
    def test_mouse_release_event_tool_false_path_calls_super_and_refresh(self) -> None:
        ...
        with (
            mock.patch.object(
                QGraphicsView, "mouseReleaseEvent", new=mock.Mock(return_value=None)
            ) as base_release,
        ):
            CanvasView.mouseReleaseEvent(
                view, _FakeEvent(button=Qt.MouseButton.LeftButton)
            )
    ...
    def test_viewport_event_generic_passthrough_and_event_false_paths_use_super(
        ...
        with mock.patch.object(
            QGraphicsView, "viewportEvent", new=mock.Mock(return_value=True)
        ) as base_viewport:
            view = self._new_view()

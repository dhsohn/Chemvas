...
def test_live_note_keys_take_at_most_one_full_snapshot(drawing, tmp_path, monkeypatch):
    ...
    snapshot = Mock(wraps=session.snapshot_state)
    monkeypatch.setattr(session, "snapshot_state", snapshot)
...
def test_returning_note_to_saved_text_keeps_other_dirty_content(
    ...
    if recovered:
        ...
        canvas.services.canvas_atom_mutation_service.add_atom("N", 180, 100)
    QTest.keyClicks(canvas, " changed")
    _key(canvas, Qt.Key.Key_Z, Qt.KeyboardModifier.ControlModifier)
    assert window.isWindowModified()
...
def test_authoritative_dirty_check_sees_unobserved_mutations(drawing, tmp_path):
    ...
    canvas.services.canvas_atom_mutation_service.add_atom("O", 170, 100)
    service = window.services.canvas_document_service
...
def test_failed_save_keeps_the_previous_note_baseline(drawing, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        canvas.services.canvas_document_session_service, "save_to_file", write
    )
    message_box = Mock()
    actions = window.services.document_action_service

...
class HistoryCommandTest(unittest.TestCase):
    ...
    def test_history_canvas_restore_marks_persistent_renderer_setter_as_partial(
        ...
        self.assertFalse(result.authoritative)
        self.assertFalse(result.fallback_to_inverse)
        self.assertEqual(canvas_snapshot.restore_calls, 2)
        self.assertEqual(renderer.setter_calls, 2)
        self.assertTrue(
            any(
    ...
    def test_history_canvas_restore_reverses_cross_authority_writers(self) -> None:
        ...
        canvas_state = {"value": "captured"}

        class _CrossWritingRenderer:
            def __init__(self) -> None:
                self._style = object()
                self.setter_calls = 0
            ...
            def style(self, value) -> None:
                self.setter_calls += 1
                self._style = value
                canvas_state["value"] = "corrupted-by-style"

        renderer = _CrossWritingRenderer()
        captured_style = renderer.style

        class _PublishingCanvasSnapshot:
            def __init__(self) -> None:
                self.restore_calls = 0
            ...
            def restore_with_result(self) -> HistoryTransactionRestoreResult:
                ...
                if self.restore_calls == 1:
                    # The external publication is one-shot and can poison an
                    # authority that was restored before the canvas snapshot.
                    renderer._style = object()
                return HistoryTransactionRestoreResult(authoritative=True)

            def _verify_exact_authorities(self) -> list[BaseException]:
                if canvas_state["value"] == "captured":
                    return []
                return [RuntimeError("renderer setter corrupted canvas state")]

        canvas_snapshot = _PublishingCanvasSnapshot()
        style_access = _capture_renderer_style_access(renderer)
        assert style_access is not None
        renderer._style = object()
        snapshot = _HistoryCanvasTransactionSnapshot(
            canvas_snapshot=canvas_snapshot,
            renderer_style=style_access,
        )
        ...
        self.assertEqual(canvas_snapshot.restore_calls, 2)
        self.assertEqual(renderer.setter_calls, 2)
        self.assertIs(renderer.style, captured_style)
        self.assertEqual(canvas_state["value"], "captured")
        self.assertTrue(
            any("corrupted canvas state" in str(error) for error in result.errors)
        )
    ...
    def test_history_canvas_rechecks_canvas_after_style_getter(self) -> None:
        ...
        for behavior, authoritative in (
            ...
            with self.subTest(behavior=behavior):
                ...
                class _GetterCorruptingRenderer:
                    def __init__(self, target_behavior, target_state) -> None:
                        self._style = object()
                        self.armed = False
                        self.corruptions = 0
                        self.behavior = target_behavior
                        self.canvas_state = target_state

                    @property
                    def style(self):
                        if self.armed and (
                            self.behavior == "persistent" or self.corruptions == 0
                        ):
                            self.corruptions += 1
                            self.canvas_state["value"] = "corrupted-by-style-getter"
                        return self._style
                ...
                renderer = _GetterCorruptingRenderer(behavior, canvas_state)
                captured_style = renderer.style
                style_access = _capture_renderer_style_access(renderer)
                assert style_access is not None
                renderer._style = object()
                ...
                class _CanvasSnapshot:
                    ...
                    def restore_with_result(
                        ...
                        self.restore_calls += 1
                        self.canvas_state["value"] = "captured"
                        return HistoryTransactionRestoreResult(authoritative=True)

                    def _verify_exact_authorities(
                        self,
                    ) -> list[BaseException]:
                        if self.canvas_state["value"] == "captured":
                            return []
                        return [RuntimeError(self.canvas_state["value"])]

                canvas_snapshot = _CanvasSnapshot(canvas_state)
                snapshot = _HistoryCanvasTransactionSnapshot(
                    canvas_snapshot=canvas_snapshot,
                    renderer_style=style_access,
                )
                ...
                self.assertTrue(
                    any(
                        "corrupted-by-style-getter" in str(error)
                        for error in result.errors
                    )
                )
        ...
        class _StyleRenderer:
            ...
            def style(self):
                ...
                if self.behavior == "fail_once_getter" and self.getter_calls == 2:
                    raise KeyboardInterrupt("transient renderer style getter failure")
                if self.behavior == "persistent_getter" and self.getter_calls > 1:
                    raise SystemExit("persistent renderer style getter failure")
                return self._style
            ...
            def style(self, value) -> None:
                ...
                if self.behavior == "fail_once_setter" and self.setter_calls == 1:
                    raise KeyboardInterrupt("transient renderer style setter failure")
                if self.behavior == "no_op":
                    return
                self._style = value

        for behavior, authoritative in (
            ("fail_once_setter", True),
            ("fail_once_getter", True),
            ("persistent_getter", False),
            ("no_op", False),
        ):
            with self.subTest(behavior=behavior):
                renderer = _StyleRenderer(behavior)
                captured_style = renderer.style
                # The helper owns the sole live capture read. Reset the count
                # after obtaining an identity solely for this assertion.
                renderer.getter_calls = 0
                style_access = _capture_renderer_style_access(renderer)
                assert style_access is not None
                self.assertIs(style_access.value, captured_style)
                self.assertEqual(renderer.getter_calls, 1)
                renderer._style = object()
                snapshot = _HistoryCanvasTransactionSnapshot(
                    canvas_snapshot=_CompletedCanvasSnapshot(),
                    renderer_style=style_access,
                ...
                if authoritative:
                    self.assertIs(renderer._style, captured_style)
                if behavior == "no_op":
                    self.assertTrue(
                        all(
                            "did not restore the captured object" in str(error)
                            for error in result.errors
                        )
                    )
    ...
    def test_broken_add_note_cannot_replace_history_primary(self) -> None:
        class BrokenNotePrimary(KeyboardInterrupt):
            def add_note(self, _note: str) -> None:
                raise SystemExit("broken add_note")

        primary = BrokenNotePrimary("position apply interrupted")
        ...
        class Port(_StatefulHistoryPort):
            ...
            def set_atom_positions_for_history(
                ...
                *,
                ...
                del canvas, positions, update_selection, coords_3d
                self.calls += 1
                if self.calls == 1:
                    raise primary
    ...
    def test_history_failure_marker_rejects_hostile_tuple_subclass(self) -> None:
        replacement = KeyboardInterrupt("hostile marker tuple decode")

        class HostileMarker(tuple):
            def __len__(self) -> int:
                raise replacement

        operation_token = object()
        primary = SystemExit("history command terminated")
        primary._chemvas_authoritative_history_transaction_restore = (  # type: ignore[attr-defined]
            HostileMarker((operation_token, True))
        )

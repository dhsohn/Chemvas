...
class _FakeOwner:
    def __init__(self) -> None:
        ...
        self.atom_point = mock.Mock(
            side_effect=lambda atom_id: QPointF(float(atom_id), float(atom_id + 1))
        )
        ...
        self.regular_ring_points_for_atom = mock.Mock(
            return_value=([QPointF(1.0, 1.0)], [(7, 1.0, 1.0)])
        )
        self.regular_ring_points_for_bond = mock.Mock(
            return_value=([QPointF(2.0, 2.0)], [(1, 2.0, 2.0)])
        )
        self.cyclohexane_chair_points = mock.Mock(
            return_value=[QPointF(1.0, 2.0), QPointF(3.0, -4.0)]
        )
        self.template_points_for_bond = mock.Mock(
            return_value=([QPointF(5.0, 5.0)], [(1, 5.0, 5.0)])
        )
        self.add_ring_from_points = mock.Mock()
        self.bond_placement_context = mock.Mock(
            return_value=SimpleNamespace(midpoint=QPointF(9.0, 10.0))
        )
...
def _actions_for(owner: _FakeOwner) -> StructureGrowthBuildActions:
    return StructureGrowthBuildActions(
        ...
        sprout_bond_endpoint=lambda atom_id, *, cyclic=False: (
            owner.sprout_bond_endpoint(
                atom_id,
                cyclic=cyclic,
            )
        ),
        add_bond_between_points=lambda start, end, style, order: (
            owner.add_bond_between_points(
                start,
                end,
                style,
                order,
        ...
        add_benzene_ring=lambda center, **kwargs: owner.add_benzene_ring(
            center, **kwargs
        ),
        has_atom=lambda atom_id: owner.has_atom(atom_id),
        default_bond_endpoint=lambda start, start_atom_id: owner.default_bond_endpoint(
            start, start_atom_id
        ),
        add_atom_label=lambda atom_id, element, **kwargs: (
            owner.committer.add_atom_label(
                atom_id,
                element,
                **kwargs,
            )
        ...
        regular_ring_points_for_atom=lambda n, atom_id: (
            owner.regular_ring_points_for_atom(n, atom_id)
        ),
        regular_ring_points_for_bond=lambda n, bond_id, midpoint: (
            owner.regular_ring_points_for_bond(
                n,
                bond_id,
                midpoint,
            )
        ...
        template_points_for_bond=lambda points_local, bond_id, midpoint: (
            owner.template_points_for_bond(
                points_local,
                bond_id,
                midpoint,
            )
        ),
        add_ring_from_points=lambda points, **kwargs: owner.add_ring_from_points(
            points, **kwargs
        ),
        bond_placement_context=lambda bond_id: owner.bond_placement_context(bond_id),
        run_recorded_additions_action=lambda action: (
            owner._run_recorded_additions_action(action)
        ),
    )
...
class StructureGrowthBuildServiceTest(unittest.TestCase):
    ...
    def test_sprout_acetyl_builds_carbonyl_and_labels_oxygen(self) -> None:
        ...
        owner.atom_point.side_effect = lambda atom_id: QPointF(
            {1: 10.0, 2: 20.0}.get(atom_id, 0.0), 0.0
        )
    ...
    def test_sprout_dimethyl_adds_two_single_bonds_from_hotspot(self) -> None:
        ...
        owner.sprout_bond_endpoint.side_effect = [
            QPointF(20.0, 0.0),
            QPointF(20.0, 10.0),
        ]

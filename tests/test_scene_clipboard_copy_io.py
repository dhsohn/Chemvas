...
@unittest.skipUnless(QApplication is not None, "PyQt6 is required for scene clipboard copy I/O tests")
class SceneClipboardCopyIOTest(unittest.TestCase):
    ...
    def test_render_clipboard_raster_image_uses_plan_dimensions_and_scale(self) -> None:
        ...
        plan = ClipboardCopyPlan(QRectF(0.0, 0.0, 12.0, 8.0), scale=2.0, image_width=24, image_height=16, payload_json=None)
    ...
    def test_build_clipboard_mime_data_sets_raster_vector_and_payload_formats(self) -> None:
        scene = QGraphicsScene()
        ...
        self.assertEqual(bytes(mime_data.data("application/x-test-selection")), b'{"format":"chemvas-selection"}')

    def test_set_vector_clipboard_data_ignores_empty_bounds_and_render_errors(self) -> None:
        scene = QGraphicsScene()
        canvas = _FakeCanvas(scene)
        mime_data = QMimeData()

        set_vector_clipboard_data(mime_data, canvas=canvas, items=[], bond_line_width=1.0)

        self.assertFalse(mime_data.hasFormat(CLIPBOARD_SVG_MIME))
        self.assertFalse(mime_data.hasFormat(CLIPBOARD_PDF_MIME))

        item = QGraphicsRectItem(0.0, 0.0, 10.0, 10.0)
        with mock.patch("ui.scene_clipboard_copy_io.render_canvas_selection_vector_bytes", side_effect=RuntimeError):
            set_vector_clipboard_data(mime_data, canvas=canvas, items=[item], bond_line_width=1.0)

...
def test_path_precheck_does_not_block_a_mapped_multicomponent_step() -> None:
    state = _document_state()
    ...
    assert readiness.single_component_endpoints is False
    assert readiness.ready_for_path_endpoints is True
    assert readiness.blocking_reasons == ()


def test_path_precheck_blocks_only_on_mapping_charge_and_multiplicity() -> None:
    state = _document_state()
    raw_plan = _plan(complete_mapping=False)
    raw_plan["states"][1]["multiplicity"] = 3  # type: ignore[index]
    plan = validate_calculation_plan(state, raw_plan)
    step = calculation_step_by_id(plan, "S01")

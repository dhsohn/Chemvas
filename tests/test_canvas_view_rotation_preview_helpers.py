...
def _controller_for(view, scene_transform=None):
    ...
    return CanvasRotationPreviewController(
        view, scene_transform_controller=scene_transform
    )
...
class CanvasViewRotationPreviewHelperTest(unittest.TestCase):
    ...
    def test_begin_selection_rotation_rejects_existing_group_empty_selection_and_missing_center(
        self,
    ) -> None:
        ...
        blocked_view = SimpleNamespace(
            rotation_preview_state=CanvasRotationPreviewState(group=object())
        )
        blocked_view.services = SimpleNamespace(
            rotation_preview_controller=_controller_for(blocked_view, blocked_transform)
        )
        self.assertFalse(
            blocked_view.services.rotation_preview_controller.begin_selection_rotation()
        )
        ...
        self.assertFalse(
            empty_view.services.rotation_preview_controller.begin_selection_rotation()
        )
    ...
    def test_begin_selection_rotation_builds_group_from_transform_preview(self) -> None:
        ...
        view.services = SimpleNamespace(
            rotation_preview_controller=_controller_for(view, transform)
        )

        self.assertTrue(
            view.services.rotation_preview_controller.begin_selection_rotation()
        )
    ...
    def test_position_preview_snapshots_apply_during_drag_and_restore_before_commit(
        self,
    ) -> None:
        ...
        transform.rotation_position_preview_snapshots.assert_called_once_with(
            [mark_item]
        )
    ...
    def test_update_rotation_preview_is_noop_without_group_and_updates_group_angle(
        self,
    ) -> None:
        idle_view = SimpleNamespace(rotation_preview_state=CanvasRotationPreviewState())
        idle_view.services = SimpleNamespace(
            rotation_preview_controller=_controller_for(idle_view)
        )
        ...
        active_view = SimpleNamespace(
            rotation_preview_state=CanvasRotationPreviewState(group=group)
        )
        active_view.services = SimpleNamespace(
            rotation_preview_controller=_controller_for(active_view)
        )
        active_view.services.rotation_preview_controller.update_rotation_preview(37.5)
        self.assertEqual(group.rotations, [37.5])

    def test_commit_selection_rotation_destroys_group_and_only_rotates_for_non_zero_angle(
        self,
    ) -> None:
        idle_view = SimpleNamespace(rotation_preview_state=CanvasRotationPreviewState())
        idle_view.services = SimpleNamespace(
        ...
        zero_view.services = SimpleNamespace(
            rotation_preview_controller=_controller_for(zero_view, zero_transform)
        )
        ...
        angle_view.services = SimpleNamespace(
            rotation_preview_controller=_controller_for(angle_view, angle_transform)
        )

...
if QApplication is not None:
    ...
    from ui.selection_outline_service import SelectionOutlineService
...
@unittest.skipUnless(QApplication is not None, "PyQt6 is required for selection outline service tests")
class SelectionOutlineServiceTest(unittest.TestCase):
    ...
    def test_selection_path_helpers_cover_bond_and_object_paths(self) -> None:
        ...
        self.assertFalse(service.selection_line_stroke_path(QPointF(0.0, 0.0), QPointF(10.0, 0.0), 4.0).isEmpty())
        self.assertFalse(service.selection_path_for_bond_item(line_item, width=4.0).isEmpty())
        self.assertFalse(service.selection_path_for_bond_item(polygon_item).isEmpty())
        self.assertFalse(service.selection_path_for_bond_item(filled_path_item).isEmpty())
        self.assertFalse(service.selection_path_for_bond_item(stroked_path_item).isEmpty())
        ...
        canvas.bond_items[0] = [QGraphicsLineItem(0.0, -2.0, 10.0, -2.0), QGraphicsLineItem(0.0, 2.0, 10.0, 2.0)]
        self.assertFalse(service.selection_path_for_bond(0).isEmpty())

        self.assertFalse(service.selection_path_for_object_item(_FakeItem("mark")).isEmpty())
        ...
        self.assertFalse(service.selection_path_for_object_item(arrow_path_item).isEmpty())
        self.assertFalse(service.selection_path_for_object_item(text_item).isEmpty())
        self.assertFalse(service.selection_path_for_object_item(empty_shape_item).isEmpty())
    ...
    def test_overlay_adders_append_scene_outlines(self) -> None:
        ...
        canvas = _FakeCanvas(
            renderer=SimpleNamespace(style=SimpleNamespace(bond_line_width=1.0, bond_length_px=20.0, bond_spacing_px=4.0)),
            scene=lambda: scene,
            model=SimpleNamespace(atoms={1: Atom("C", 0.0, 0.0)}, bonds=[Bond(1, 1, 1)]),
            services=SimpleNamespace(
                geometry_controller=SimpleNamespace(
                    ring_center_for_bond=lambda bond: None,
                    trim_line_for_labels=lambda *_args: (0.0, 1.0),
        ...
        service.selection_path_for_object_item = mock.Mock(
            return_value=service.selection_line_stroke_path(QPointF(0.0, 0.0), QPointF(5.0, 0.0), 3.0)
        )

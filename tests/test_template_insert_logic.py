...
def _make_resolvers(**overrides) -> TemplatePointResolvers:
    resolvers = TemplatePointResolvers(
        ...
        ring_points=Mock(return_value=_points(6)),
        regular_ring_points_for_atom=Mock(return_value=_points(6, start=12.0)),
        regular_ring_points_for_bond=Mock(return_value=_points(6, start=5.0)),
        chair_points=Mock(return_value=_points(6, start=-6.0)),
        chair_flipped_points=Mock(return_value=_points(6, start=-12.0)),
        boat_points=Mock(return_value=_points(6, start=-2.0)),
        template_points_for_bond=Mock(return_value=_points(6, start=9.0)),
    )
...
class TemplateInsertLogicTest(unittest.TestCase):
    ...
    def test_resolve_free_regular_ring_uses_regular_radius(self) -> None:
        ...
        self.assertEqual(resolution.points, _points(5))
    ...
    def test_resolve_bond_template_shape_builds_local_points_before_projection(self) -> None:
        ...
        chair_points = Mock(return_value=local_points)
        template_points_for_bond = Mock(return_value=projected_points)
        resolvers = _make_resolvers(
        ...
        assert resolution is not None
        self.assertEqual(resolution.points, projected_points)
    ...
    def test_resolve_free_template_shape_uses_cursor_position_as_center(self) -> None:
        ...
        assert resolution is not None
        self.assertEqual(resolution.points, _points(6, start=20.0))
    ...
    def test_resolve_bond_regular_ring_delegates_to_bond_resolver(self) -> None:
        ...
        assert resolution is not None
        self.assertEqual(resolution.points, _points(7, start=30.0))
    ...
    def test_resolve_rejects_degenerate_or_wrong_size_point_lists(self) -> None:
        ...
        assert plan is not None
        for points in (_points(2), _points(5), _points(7)):
            with self.subTest(points=points):
                resolvers = _make_resolvers(ring_points=Mock(return_value=points))
                self.assertIsNone(resolve_template_insert(request, plan, resolvers))

...
import chemvas.ui.tool_service_bundle as tool_service_bundle
...
def test_build_tool_services_injects_tool_context_ports(monkeypatch) -> None:
    ...
    assert (
        services.tools.kwargs["selection_rotation_controller"]
        is selection_rotation_controller
    )
    assert services.tools.kwargs["scene_delete_controller"] is scene_delete_controller
    assert (
        services.tools.kwargs["scene_transform_controller"]
        is scene_transform_controller
    )
    assert services.tools.kwargs["style_controller"] is style_controller
    assert (
        services.tools.kwargs["bond_sets_for_atoms"]
        is graph_service.bond_sets_for_atoms
    )
    ...
    assert (
        services.tools.kwargs["rubber_band_drag_mode"] is canvas.DragMode.RubberBandDrag
    )

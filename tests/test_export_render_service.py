...
class ExportRenderServiceTest(unittest.TestCase):
    ...
    def test_svg_viewport_extraction_preserves_bytes_at_rounding_boundaries(
        ...
        def previous_configuration(generator, plan, title):
            ...
            generator.setSize(
                QSize(max(1, round(plan.out_w_pt)), max(1, round(plan.out_h_pt)))
            )
            generator.setViewBox(QRectF(0, 0, plan.out_w_pt, plan.out_h_pt))
            if title:
        ...
        for width in (71.49, 71.5, 72.5, 72.51):
            plan = ExportPlan(
                source_x=0,
                source_y=0,
                source_w=100,
            ...
            with self.subTest(width=width):
                current = vector.render_svg_bytes(
                    scene, items, plan, "white", "fixture"
                )
                with mock.patch.object(
                    vector, "configure_svg_generator", previous_configuration
                ):
                    previous = vector.render_svg_bytes(
                        scene, items, plan, "white", "fixture"
                    )
